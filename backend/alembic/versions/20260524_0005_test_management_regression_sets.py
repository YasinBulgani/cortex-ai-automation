...
def upgrade() -> None:
    ...
    if "test_management_regression_sets" not in existing:
        op.create_table(
    ...
    op.create_index("ix_tm_regression_sets_project_id", "test_management_regression_sets", ["project_id"], if_not_exists=True)

    if "test_management_regression_set_cases" not in existing:
        op.create_table(
    ...
    op.create_index("ix_tm_regression_set_cases_set_id", "test_management_regression_set_cases", ["regression_set_id"], if_not_exists=True)
    op.create_index("ix_tm_regression_set_cases_case_id", "test_management_regression_set_cases", ["case_id"], if_not_exists=True)

    op.execute("ALTER TABLE test_management_runs ADD COLUMN IF NOT EXISTS source_type VARCHAR(32) NOT NULL DEFAULT 'manual'")
    op.execute("ALTER TABLE test_management_runs ADD COLUMN IF NOT EXISTS source_ref UUID")
    op.execute("ALTER TABLE test_management_runs ADD COLUMN IF NOT EXISTS scope_snapshot JSONB NOT NULL DEFAULT '{}'")
    op.execute("ALTER TABLE test_management_run_cases ADD COLUMN IF NOT EXISTS case_snapshot JSONB NOT NULL DEFAULT '{}'")
    op.execute("ALTER TABLE test_management_defect_links ADD COLUMN IF NOT EXISTS severity VARCHAR(32) NOT NULL DEFAULT 'major'")
    op.execute("ALTER TABLE test_management_defect_links ADD COLUMN IF NOT EXISTS priority VARCHAR(32) NOT NULL DEFAULT 'P2'")
    op.execute("ALTER TABLE test_management_defect_links ADD COLUMN IF NOT EXISTS assignee_id UUID")
    op.execute("ALTER TABLE test_management_defect_links ADD COLUMN IF NOT EXISTS root_cause VARCHAR(100)")
    op.execute("ALTER TABLE test_management_defect_links ADD COLUMN IF NOT EXISTS retest_status VARCHAR(32) NOT NULL DEFAULT 'not_ready'")
    op.execute("ALTER TABLE test_management_defect_links ADD COLUMN IF NOT EXISTS resolved_at TIMESTAMP WITH TIME ZONE")
    op.execute("ALTER TABLE test_management_defect_links ADD COLUMN IF NOT EXISTS verified_at TIMESTAMP WITH TIME ZONE")
    op.execute("ALTER TABLE test_management_defect_links ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()")

    op.execute("""
        DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tm_defect_links_assignee_id') THEN
                ALTER TABLE test_management_defect_links
                    ADD CONSTRAINT fk_tm_defect_links_assignee_id
                    FOREIGN KEY (assignee_id) REFERENCES sd_users(id) ON DELETE SET NULL;
            END IF;
        END $$;
    """)

    if "test_management_release_signoffs" not in existing:
        op.create_table(
    ...
    op.create_index("ix_tm_release_signoffs_project_id", "test_management_release_signoffs", ["project_id"], if_not_exists=True)

    if "test_management_requirements" not in existing:
        op.create_table(
    ...
    op.create_index("ix_tm_requirements_project_id", "test_management_requirements", ["project_id"], if_not_exists=True)

    op.execute("ALTER TABLE test_management_requirement_links ADD COLUMN IF NOT EXISTS requirement_id UUID")
    op.create_index("ix_tm_requirement_links_requirement_id", "test_management_requirement_links", ["requirement_id"], if_not_exists=True)
    op.execute("""
        DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tm_requirement_links_requirement_id') THEN
                ALTER TABLE test_management_requirement_links
                    ADD CONSTRAINT fk_tm_requirement_links_requirement_id
                    FOREIGN KEY (requirement_id) REFERENCES test_management_requirements(id) ON DELETE SET NULL;
            END IF;
        END $$;
    """)
    ...
    op.execute("""
        ...
        CREATE POLICY rls_tenant_isolation ON test_management_release_signoffs
            USING (project_id IN (SELECT id FROM test_management_projects WHERE tenant_id = current_tenant_id()::text));
