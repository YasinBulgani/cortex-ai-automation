...
class AliasIndex:
    ...
    def search(
        ...
        *,
        ...
        Args:
            query: Sorgu metni.
            lang: Eşleşmeyi belli dile kısıtla ("tr" / "en" / None=hepsi).
            k: Döndürülecek maksimum eşleşme sayısı.
            min_score: Bu eşiğin altındaki cosine skorları elenir.
            rerank: None → ``reranker.is_enabled()`` ile karar verilir.
                True → retrieval pool'u genişlet (k_in), cross-encoder ile
                yeniden sırala, ilk k'yı dön. False → saf cosine.

        Rerank açıksa retrieval havuzu ``AI_MODEL_RERANKER_TOP_K_IN`` kadar
        büyütülür (ör. 20) ve sonuç ``k`` değerine göre kırpılır. Bu sayede
        geniş recall → dar precision akışı kurulur.
        """
        q = (query or "").strip()
        if not q:
            return []
        ...
        # Rerank açıksa retrieval havuzunu genişlet: cross-encoder geniş
        # bir recall setinden dar bir precision seti damıtır.
        use_rerank = reranker.is_enabled() if rerank is None else rerank
        pool_size = k
        if use_rerank:
            import os as _os
            pool_in = int(_os.environ.get("AI_MODEL_RERANKER_TOP_K_IN", "20") or 20)
            pool_size = max(k, pool_in)
        ...
        if use_rerank and len(hits) > 1:
            # Reranker'a vereceğimiz metin: eşleşen alias + action description.
            # Bu, cross-encoder'ın query ile daha zengin bir comparison yapması
            # için (sadece kısa alias değil) — parafraz sensitivity burada kazanılır.
            def _text_for(h: SemanticHit) -> str:
                desc = (h.action.description or "").strip()
                if desc and desc != h.matched_alias:
                    return f"{h.matched_alias} — {desc}"
                return h.matched_alias

            hits = reranker.rerank(query, hits, text_of=_text_for, top_k=k)
