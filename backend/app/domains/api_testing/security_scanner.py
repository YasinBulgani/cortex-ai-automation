...
Banking API endpoint'leri için guvenlik taramasi.
...
_AUTH_PATTERNS = re.compile(
    r"(auth|login|logout|token|session|otp|mfa|2fa|password|şifre|kimlik)",
    re.IGNORECASE,
)
...
_PII_PATTERNS = re.compile(
    r"(user|kullanıcı|profile|profil|kyc|identity|tckn|email|phone|address)",
    re.IGNORECASE,
)
    ...
    if has_id and no_auth:
        ...
        findings.append({
            ...
            "banking_impact": (
                ...
                if is_banking
                else "Kullanıcı verilerine yetkisiz erisim riski."
            ),
        })
...
def _check_property_auth(
    ...
    if prop_count > 10:
        findings.append({
            ...
            "confidence": 0.7,
            "banking_impact": (
                "Hesap bakiyesi, kredi limiti veya kullanıcı rolu gibi "
...
def _check_bfla(
    ...
    if is_admin:
        ...
        findings.append({
            ...
            "banking_impact": (
                "Yetkisiz kullanıcılar admin islevlerine erisebilir — "
...
def _check_banking_compliance(
    ...
    if is_financial:
        ...
        if method in ("POST", "PUT", "PATCH", "DELETE"):
            findings.append({
                ...
                "confidence": 0.8,
                "banking_impact": (
                    "BDDK mevzuati geregi tüm finansal islemlerin kaydi zorunludur. "
    ...
    if has_pii:
        ...
        findings.append({
            ...
            "confidence": 0.7,
            "banking_impact": (
                "KVKK ihlali — kişisel verilerin korunmasi kanununa "
...
def get_security_dashboard(
    ...
    if findings_by_owasp.get("API1:2023", 0) > 0:
        recommendations.append(
            "BOLA: Tüm ID-bazli endpoint'lere nesne seviyesinde yetkilendirme eklenmelidir."
        )
    ...
    if findings_by_owasp.get("API4:2023", 0) > 0:
        recommendations.append(
            "Rate Limiting: Tüm endpoint'lere istek sinirlamasi uygulanmalidir."
        )
    if kvkk_failed > 0:
        recommendations.append(
            "KVKK: %d kişisel veri endpoint'i kimlik dogrulama gerektirir." % kvkk_failed
        )
    if bddk_failed > 0:
        recommendations.append(
