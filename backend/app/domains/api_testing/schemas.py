...
CRUD + Execution + AI Generation için request/response modelleri.
"""
...
class AIGenerateRequest(BaseModel):
    """AI ile test case üretim istegi."""
    mode: str = Field(default="test_generation")
    # test_generation, security_audit, chain_builder

    endpoint_ids: Optional[List[str]] = None  # Belirli endpoint'ler için
    spec_id: Optional[str] = None  # Tüm spec için
...
class ExecutionResultOut(BaseModel):
    """Tek bir request çalışma sonucu."""
    method: str
    url: str
...
class ExecutionHistoryItem(BaseModel):
    """Tek bir çalışma ozeti — execution history listesi için."""
    run_id: str
...
class ExecutionDetailItem(BaseModel):
    """Tek bir test case çalışma detayi — run detail için."""
    id: str
