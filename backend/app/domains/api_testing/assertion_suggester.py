...
PII_FIELDS = [
    ...
    "date_of_birth", "dogum_tarihi", "password", "şifre",
]
...
def _suggest_response_time(
    ...
    return [_make_suggestion(
        ...
        "Response süresi %dms altinda olmali (%s risk seviyesi)" % (threshold, risk_level),
    ...
    if "KVKK" in compliance_tags:
        ...
        if not kvkk_checked:
            suggestions.append(_make_suggestion(
                ...
                "KVKK: Yanıt icerisinde maskelenmemis TCKN verisi olmamali",
                PRIORITY_CRITICAL, CAT_COMPLIANCE,
            ))
    ...
    if test_type == "boundary":
        ...
        if not error_msg_checked:
            suggestions.append(_make_suggestion(
                ...
                "Sinir deger testlerinde hata mesajı alani kontrol edilmeli",
        ...
        if "response_time" not in existing:
            suggestions.append(_make_suggestion(
                ...
                "Performans testlerinde response süresi kontrolu zorunlu",
        ...
        if not id_checked:
            suggestions.append(_make_suggestion(
                ...
                "Başarılı POST/PUT yanitinda kayit ID'si donmeli",
                PRIORITY_MEDIUM, CAT_FUNCTIONAL,
            ))
...
def suggest_assertions(
    ...
    if current_count == 0 and total_after > 0:
        improvement = "Sıfır assertion'dan %d'e cikarilarak test coverage onemli olcude arttirilir" % total_after
