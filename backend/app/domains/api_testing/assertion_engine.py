...
Desteklenen assertion tipleri:
  ...
  - response_time  : Yanıt süresi (ms) kontrolu
...
class AssertionReport:
    """Tüm assertion'larin toplu sonucu."""
    total: int = 0
    passed: int = 0
    failed: int = 0
...
def _resolve_json_path(data: Any, path: str) -> Tuple[bool, Any]:
    ...
    for part in parts:
        ...
        if match:
            ...
            # Array uzerinde field erisimu — her eleman için
            results = []
            for item in current:
...
def evaluate_assertions(
    ...
    *,
    ...
    Her assertion dict'i su alanlari icerir:
      ...
      path     : json_path için yol (opsiyonel)
...
def infer_schema(response_body: Any) -> dict:
    ...
    Spec yoksa gerçek response'tan schema olusturmak için kullanilir.
    """
    try:
