...
class DeviceBroker:
    ...
    def enroll_physical(self, req: PhysicalEnrollRequest) -> Device:
        ...
        # ADB (android) / idb (ios) baglanabilirlik dogrulamasi henuz implemente edilmedi.
        # Cihaz sisteme eklendi ancak gercekten erisilebildigi dogrulanmadi.
        # Gercek ADB handshake: subprocess.run(["adb", "-s", serial, "get-state"])
        # Gercek idb: subprocess.run(["idb", "describe", "--udid", udid])
        _logger.warning(
            "Fiziksel cihaz %s (%s, %s) baglantisi dogrulanmadan enroll edildi. "
            "Test sirasinda erisim hatasi alabilirsiniz.",
            dev_id, req.platform, req.name,
        )
        return device
    ...
    # ── Health probe ─────────────────────────────────────────
    def probe_appium(self, device_id: str, timeout: float = 0.5) -> Optional[Device]:
        """Appium /status ile cihaz endpoint'inin gerçekten hazır olup olmadığını ölç."""
        dev = self.get(device_id)
        if not dev:
            return None

        try:
            client = AppiumClient(dev.appium_url, timeout=timeout)
            try:
                status = client.status()
            finally:
                client._http.close()
            if status.get("ready") is False:
                return self.update_status(device_id, "error", last_error=str(status.get("message") or "Appium ready=false"))
            return self.update_status(device_id, "idle", last_error=None)
        except Exception as exc:
            return self.update_status(device_id, "error", last_error=str(exc))

    def probe_all_appium(self, timeout: float = 0.5) -> list[Device]:
        probed: list[Device] = []
        for dev in self.list():
            updated = self.probe_appium(dev.id, timeout=timeout)
            if updated:
