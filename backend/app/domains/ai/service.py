...
def _is_local_llm_url(value: str) -> bool:
    """Return True for localhost, private IPs and docker service names."""
    ...
    try:
        ...
        # Docker Compose service names such as "ollama" or "vllm" are local-network targets.
        return "." not in host


def _assert_llm_provider_allowed(provider: str) -> None:
    """Block external LLM calls when enterprise local-only mode is active."""
    ...
    if normalized == "gateway":
        # Gateway enforces its own local-only policy — backend cannot inspect routing
        return
    if normalized == "openai" and _is_local_llm_url(settings.openai_base_url):
        return
    raise ValueError(
        "AI_LOCAL_ONLY aktif: müşteri verisi dış LLM sağlayıcısına gönderilemez. "
        "Yerel Ollama/vLLM kullanın veya bilinçli olarak AI_LOCAL_ONLY=false yapın."
    )
...
# Bu tür hatalar retry edilmez — tekrar denemek sonucu değiştirmez.
_NON_RETRIABLE_TYPES = (ValueError, TypeError, PermissionError, AttributeError, KeyError)
_NON_RETRIABLE_KEYWORDS = ("authentication", "api key", "invalid_api_key", "unauthorized", " 401 ", " 403 ", "permission")


def _is_retriable_error(exc: Exception) -> bool:
    """Geçici hata mı (retry beklenebilir) yoksa kalıcı hata mı?"""
    if isinstance(exc, _NON_RETRIABLE_TYPES):
        return False
    msg = str(exc).lower()
    if any(kw in msg for kw in _NON_RETRIABLE_KEYWORDS):
        return False  # Auth/permission hataları retry ile düzelmez
    return True


def _with_retry(func):
    """LLM cagirilari için exponential backoff retry decorator.

    Sadece geçici hatalar (timeout, rate limit, bağlantı sorunları) retry edilir.
    Auth hataları, ValueError vb. kalıcı hatalar hemen fırlatılır.
    """
    @wraps(func)
        ...
        for attempt in range(1, MAX_LLM_RETRIES + 1):
            try:
                ...
                if not _is_retriable_error(e):
                    raise  # Kalıcı hata — retry etme, hemen fırlat
                last_error = e
                if attempt < MAX_LLM_RETRIES:
                    ...
                    logger.error("LLM %s %d denemede başarısız: %s", func.__name__, MAX_LLM_RETRIES, e)
        raise last_error  # type: ignore[misc]
    return wrapper


def _with_async_retry(func):
    """Async LLM cagrilari için exponential backoff retry decorator.
        ...
        for attempt in range(1, MAX_LLM_RETRIES + 1):
            try:
                ...
                if attempt < MAX_LLM_RETRIES:
                    ...
                    logger.error("Async LLM %s %d denemede başarısız: %s", func.__name__, MAX_LLM_RETRIES, e)
        raise last_error  # type: ignore[misc]
    return wrapper
...
# Context window per Ollama model — used by both sync and async callers.
...
def call_llm(
    ...
    *,
    ...
    _assert_llm_provider_allowed(provider)
    kw: dict[str, Any] = dict(
        json_mode=json_mode, history=history,
        temperature=temperature, model=model, max_tokens=max_tokens,
    )

    # Model adini coz (trace için)
    resolved_model = model
    if not resolved_model:
        if provider == "gateway":
            resolved_model = "gateway"
        elif provider == "ollama":
            resolved_model = getattr(settings, "ollama_model_chat", None) or getattr(settings, "ollama_model_analyst", "llama3.1:8b")
    ...
    try:
        if provider == "gateway":
            ...
            result = gateway_complete(
                task_type=_trace_task_type or _trace_phase or "chat",
        ...
        try:
            ...
            logger.debug("Trace kaydi basarisiz (pipeline devam ediyor): %s", _trace_exc)
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            logger.debug("Token tahmini basarisiz: %s", _tok_exc)
        ...
        try:
            ...
            logger.debug("Async trace kaydi basarisiz (pipeline devam ediyor): %s", _trace_exc)
...
def _parse_json_response(raw: str) -> dict:
    ...
    try:
        import re as _re
        # Sadece key/value etrafindaki tek tirnaklari değiştir
        single_to_double = raw.replace("'", '"')
        return json.loads(single_to_double)
    except (json.JSONDecodeError, Exception):
        pass

    logger.error("JSON parse başarısız (5 strateji denendi): %s", raw[:500])
    raise ValueError("AI yaniti gecerli JSON formatinda degil — tüm parse stratejileri başarısız.")
...
def chat_completion_stream(
    ...
    """
    system = _build_chat_system_prompt(user_message, context)
    provider = _resolve_effective_provider()
    _assert_llm_provider_allowed(provider)
...
def _stream_gateway(
    ...
    *,
    ...
    url = f"{_gateway_base()}/ai/stream"
    try:
        with httpx.stream("POST", url, json=payload, headers=_gateway_headers(), timeout=90.0) as resp:
...
def _get_async_http_client() -> "httpx.AsyncClient":
    ...
    if _async_http_client is None or _async_http_client.is_closed:
        _async_http_client = _httpx.AsyncClient(
            timeout=_httpx.Timeout(90.0, connect=5.0),
            limits=_httpx.Limits(max_connections=30, max_keepalive_connections=15),
        )
    return _async_http_client


async def _async_stream_gateway(
    messages: list[dict],
    *,
    task_type: str = "chat",
    temperature: float | None = None,
    ...
    try:
        ...
        raise RuntimeError("Gateway streaming için httpx gerekli") from exc
...
async def async_stream_llm(
    ...
    *,
    ...
    if provider == "gateway":
        async for token in _async_stream_gateway(
            messages=messages,
            task_type=_trace_task_type or _trace_phase or "chat",
            temperature=temperature,
            max_tokens=max_tokens,
