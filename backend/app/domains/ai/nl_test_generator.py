"""
NL Test Generator — Dogal Dil Aciklamalarindan Test Üretim Pipeline'i
=====================================================================
...
_TEST_TYPE_INDICATORS = {
    "positive": [
        "başarılı", "gecerli", "dogru", "200", "201", "success",
        "dogrula", "donmeli", "calisir", "calismal",
    ],
    "negative": [
        "hatasi", "hata", "geçersiz", "eksik", "bos", "yanlis",
        "400", "401", "403", "404", "500",
...
_METHOD_INDICATORS = {
    ...
    "POST": ["oluştur", "ekle", "gönder", "kaydet", "transfer", "post", "create", "submit", "yap"],
    "PUT": ["guncelle", "değiştir", "update", "put", "modify"],
    "PATCH": ["kismi", "patch", "partial"],
    "DELETE": ["sil", "kaldır", "delete", "remove", "iptal"],
}
...
class NLTestGenerator:
    """
    Dogal dil aciklamalarindan test üretim servisi.
    ...
    def generate_from_text(
        ...
        Returns:
            Pipeline sonuç dict'i — parsed_intent, matched_endpoints,
        ...
        try:
            ...
            validation["warnings"].append("LLM üretim hatasi: %s" % str(exc)[:200])
    ...
    def suggest_from_endpoint(
        ...
        """
        Verilen endpoint için QA muhendisinin yazabilecegi
        dogal dil test aciklamalari öner.

        Args:
            endpoint_id: ApiEndpoint kayit ID'si
            count:       Kac öneri uretilsin (default 5)
    ...
    def validate_code(self, code: str, language: str) -> Dict[str, Any]:
        ...
        if language == "python":
            ...
            if open_count != close_count:
                errors.append(
                    "Dengesiz suslu parantez: %d açık, %d kapalı" % (open_count, close_count)
                )
        else:
    ...
    def _normalize_turkish(self, text: str) -> str:
        """Turkce karakterleri ASCII'ye normalize et (arama için)."""
        replacements = {
            "\u00e7": "c", "\u011f": "g", "\u0131": "i", "\u00f6": "o",
            "\u015f": "s", "\u00fc": "u",
            "\u00c7": "C", "\u011e": "G", "\u0130": "I", "\u00d6": "O",
    ...
    def _extract_status(self, text: str) -> Optional[int]:
        ...
        if "başarılı" in text or "donmeli" in text:
            # Check for error keywords
            if any(kw in text for kw in ["hata", "yetersiz", "geçersiz", "yetkisiz"]):
    ...
    def _extract_conditions(self, text: str) -> List[str]:
        ...
        condition_patterns = [
            ...
            (r"(?:yetersiz|geçersiz|eksik|bos)\s+(\w+)", "invalid_field"),
        ]
    ...
    def _build_system_prompt(self, output_format: str) -> str:
        """Output formatina gore system prompt oluştur."""
        base = (
        ...
        if output_format == "api_test":
            return base + (
                "\nJSON formatinda test case'ler üret. Her test case su alanlari icermeli:\n"
                "- title: Test basligi\n"
            ...
            return base + (
                "\nGherkin (BDD) formatinda Turkce senaryo üret.\n"
                "Özellik/Senaryo/Verilen/Ve/Zaman/O zaman kaliplarini kullan.\n"
                "Yanitini su JSON formatinda ver:\n"
                '{"gherkin": "Özellik: ...\\nSenaryo: ...\\n..."}\n'
            )
        elif output_format == "pytest":
            return base + (
                "\nPython pytest kodu üret. requests kutuphanesini kullan.\n"
                "Her test fonksiyonu için assert ifadeleri ekle.\n"
            ...
            return base + (
                ...
                "API testing için request fixture'ini kullan.\n"
                "Yanitini su JSON formatinda ver:\n"
    ...
    def _llm_suggest(self, endpoint: Any, count: int) -> List[Dict[str, str]]:
        """LLM kullanarak endpoint için test onerileri üret."""
        from app.domains.ai.service import call_llm

        system = (
            "Sen bankacilik test uzmanisisin. Verilen API endpoint için "
            "bir QA muhendisinin dogal dilde yazabilecegi test aciklamalari öner.\n"
            "Her öneri için JSON formatinda su alanlari doldur:\n"
    ...
    def _rule_based_suggestions(self, endpoint: Any, count: int) -> List[Dict[str, str]]:
        """Kural tabanli fallback: LLM başarısız olursa basit oneriler üret."""
        suggestions = []  # type: List[Dict[str, str]]
        ...
        # Positive test
        suggestions.append({
            "text": "%s %s endpoint'ine gecerli parametrelerle istek gonderildiginde başarılı yanıt donmeli" % (method, path),
            "test_type": "positive",
            "priority": "P1",
        })
        ...
        if method in ("POST", "PUT", "PATCH"):
            suggestions.append({
                "text": "%s %s endpoint'ine bos body gonderildiginde uygun hata mesajı donmeli" % (method, path),
                "test_type": "boundary",
                "priority": "P2",
            })
