...
Her agent kendi çalışma sonuclarini yapilandirilmis sekilde kaydeder.
...
Ornek akis:
  ...
  4. SelfImproving → tüm agent'larin sonuclarini meta-analiz yapar

Kaynak tipleri:
  agent_output   — Agent çalışma sonuclari (yapilandirilmis)
...
class CrossAgentMemory:
    ...
    def query(
        ...
        Args:
            requesting_agent: Sorgulayan agent (log için)
            event_types: Filtrelenecek olay tipleri
            tags: En az birinin eslesmesi gereken etiketler
            exclude_agent: Haric tutulacak agent (kendi ciktisini haric tutmak için)
            limit: Maks sonuç sayisi
    ...
    def get_context_for_agent(
        ...
        """
        Belirli bir agent için zenginlestirilmis context oluştur.

        Diger agent'larin ciktilarina gore özet bir metin uretir.
        """
        entries = cls.query(
            project_id=project_id,
    ...
    def get_risk_findings(cls) -> List[Dict[str, Any]]:
        """Tüm risk bulgularini getir."""
        return cls.query(event_types=["risk_finding"])

    @classmethod
    def get_quality_scores(cls) -> List[Dict[str, Any]]:
        """Tüm kalite puanlarini getir."""
        return cls.query(event_types=["quality_score"])

    @classmethod
    def get_failure_insights(cls) -> List[Dict[str, Any]]:
        """Tüm hata insight'larini getir."""
        return cls.query(event_types=["failure_insight"])
    ...
    def _format_for_storage(entry: Dict[str, Any]) -> str:
        """Entry'yi KnowledgeStore için metin formatina cevir."""
        parts = [
