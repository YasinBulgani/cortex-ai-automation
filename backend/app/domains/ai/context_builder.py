...
Faz 2 iyilestirmeleri:
    * Stop word listesi dosyadan (data/turkish_stopwords.txt) yuklenir.
    * Token-aware butce (~4 char/token).
    * Query rewriting: TR/EN deterministik es anlam genisletme.
    * KnowledgeStore.retrieve_hybrid entegrasyonu (RAG snippet tipi).
"""
...
# ── Stop words dosyadan yuklenir ─────────────────────────────────────────

_STOP_WORDS_FALLBACK = {
    "acaba", "ama", "ancak", "bir", "bu", "da", "de", "daha", "değil",
    "diye", "en", "gibi", "hangi", "için", "ile", "ilgili", "mi", "mı",
    "mu", "mü", "nasıl", "ne", "neden", "olan", "olarak", "the", "and",
    "or", "for", "to", "of", "in", "is", "are", "how", "what", "why",
    "ve", "veya", "ya",
}
...
def _load_stop_words() -> frozenset[str]:
    """data/turkish_stopwords.txt'den yükle; yoksa fallback."""
    try:
        data_path = Path(__file__).parent / "data" / "turkish_stopwords.txt"
        if not data_path.exists():
            return frozenset(_STOP_WORDS_FALLBACK)
        words: set[str] = set()
        ...
        if not words:
            return frozenset(_STOP_WORDS_FALLBACK)
        logger.debug("stopwords yuklendi: %d kelime", len(words))
        return frozenset(words)
    except Exception as exc:
        logger.warning("stopwords yuklenemedi: %s", exc)
        return frozenset(_STOP_WORDS_FALLBACK)
...
# ── Query rewriting: TR/EN + banka jargon karsiliklari ──────────────────

_SYNONYMS: dict[str, tuple[str, ...]] = {
    # auth
    "login": ("giriş", "oturum", "auth"),
    "giriş": ("login", "auth"),
    "auth": ("login", "giriş", "kimlik"),
    "logout": ("cikis",),
    "cikis": ("logout",),
    "password": ("şifre", "parola"),
    "şifre": ("password", "parola"),
    # money/financial
    "transfer": ("havale", "eft", "para"),
    "havale": ("transfer", "eft"),
    "eft": ("transfer", "havale"),
    "payment": ("odeme", "fatura"),
    "odeme": ("payment",),
    "account": ("hesap",),
    "hesap": ("account",),
    "balance": ("bakiye",),
    ...
    "kart": ("card",),
    # QA domain
    "failure": ("hata", "bug", "sorun", "fail"),
    "hata": ("error", "failure", "bug"),
    "error": ("hata",),
    "flaky": ("kararsiz", "tutarsiz"),
    "coverage": ("kapsam",),
    "kapsam": ("coverage",),
...
def _expand_query(keywords: list[str]) -> list[str]:
    """Anahtar kelimelere es anlamlilari ekle — retrieval recall artirir."""
    expanded: list[str] = []
    seen: set[str] = set()
    for kw in keywords:
        key = kw.lower()
...
def build_project_ai_context(
    ...
    *,
    ...
    Args:
        max_chars: Geriye donuk uyumluluk; verilirse max_tokens'i ezer.
        max_tokens: Hedef token butcesi (default 4000 ~ 16000 char).
        include_rag: KnowledgeStore RAG snippet'lerini ekle (Faz 3).
    """
    ...
    if max_chars is not None:
        max_tokens = max_chars // _APPROX_CHARS_PER_TOKEN
    ...
    snippets = _collect_relevant_snippets(db, project_id, query, expanded_keywords, intent)

    # Faz 3: KnowledgeStore RAG snippet'lerini hibrit olarak ekle
    if include_rag and query:
        try:
            rag_snippets = _collect_rag_snippets(query, keywords, intent, top_k=3)
            snippets.extend(rag_snippets)
        except Exception as exc:
            logger.debug("RAG snippet alinamadi: %s", exc)

    snippets = sorted(snippets, key=lambda s: (-s.score, s.source_type, s.title))

    if snippets:
        snippet_lines = ["Soru ile en ilgili proje kayıtları:"]
        current_tokens = sum(_approx_tokens(s) for s in sections) + _approx_tokens(
            "\n\n".join(snippet_lines)
        )
        for snippet in snippets:
            block = f"[{snippet.source_type}] {snippet.title}\n{snippet.body}"
            projected_tokens = current_tokens + _approx_tokens(block) + 1
            if projected_tokens > max_tokens and len(snippet_lines) > 1:
                break
...
def _collect_rag_snippets(
    ...
    if intent == "failure":
        source_filter = ["execution", "error_pattern", "insight"]
    elif intent == "automation":
        source_filter = ["feature_file", "code_change", "insight"]

    store = KnowledgeStore()
    try:
        retrieve_fn = getattr(store, "retrieve_hybrid", store.retrieve)
        chunks = retrieve_fn(query, top_k=top_k, sources=source_filter, min_similarity=0.30)
    except Exception as exc:
        logger.debug("KnowledgeStore.retrieve hatasi: %s", exc)
        return []

    snippets: list[ContextSnippet] = []
    for chunk in chunks or []:
        rag_score = 3.0 + float(getattr(chunk, "similarity", 0.5))
        title = str(chunk.metadata.get("title") or chunk.source or "RAG")[:80]
        body_lines = [
            f"Benzerlik: %{(getattr(chunk, 'similarity', 0.5) * 100):.0f}",
            _truncate(chunk.content or "", 400),
        ]
        if chunk.metadata:
            meta_str = ", ".join(
                f"{k}={v}" for k, v in list(chunk.metadata.items())[:3] if k != "title"
            )
            if meta_str:
                body_lines.append(f"Meta: {meta_str}")
        snippets.append(
            ContextSnippet(
                source_type=f"RAG/{chunk.source}",
                title=title,
                body="\n".join(body_lines),
                score=rag_score,
            )
