...
class QAOrchestrator:
    ...
    def plan(self, goal: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        system_prompt = (
            ...
            "Verilen hedefe ulasmak için adım adım bir test plani oluştur. "
            "Her adım için uygun aksiyonu sec.\n\n"
            "KULLANILABILIR AKSIYONLAR:\n"
            "- analyze_coverage: Mevcut test kapsamini analiz et\n"
            "- generate_tests: Eksik testleri AI ile üret\n"
            "- execute_tests: Testleri çalıştır\n"
            "- analyze_results: Test sonuclarini LLM ile analiz et\n"
            "- heal_failures: Başarısız testleri self-healing ile duzelt\n"
            "- suggest_assertions: Eksik assertion onerileri üret\n"
            "- detect_flaky: Flaky testleri tespit et\n"
            "- prioritize: Testleri onceliklendir\n"
            "- report: Özet rapor oluştur\n\n"
            "YALNIZCA JSON formatinda yanıt ver."
        )
    ...
    def _default_plan(self, gaps: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if gaps:
            steps.append({
                ...
                "description": "Kapsam bosluklari için yeni testler üret",
                "target": None,
            ...
            steps.append({
                ...
                "description": "Mevcut testler için eksik assertion onerileri",
                "target": None,
                "params": {},
                "depends_on": [1],
    ...
    def _generate_recommendations(
        ...
        if coverage_rate < 50:
            ...
            recommendations.append(
                "Kapsam %%%.1f — hedef %%80+ için eksik alanlara odaklanin"
                % coverage_rate
            )
    ...
    def explore(self, spec_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        goal_parts = []
        if critical > 0:
            goal_parts.append("%d kritik endpoint için test oluştur" % critical)
        if uncovered > 0:
            goal_parts.append("%d kapsamda olmayan endpoint için test üret" % uncovered)
        if not goal_parts:
