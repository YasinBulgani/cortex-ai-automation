"""JWT ve parola yardimcilari — token olusturma, dogrulama, şifre hashleme."""
...
def create_access_token(
    ...
    *,
    ...
    payload = {
        ...
        "jti": secrets.token_hex(8),  # Token ID — revocation için
    }
    if extra_claims:
...
def verify_password_reset_token(token: str) -> Optional[str]:
    """Şifre sifirlama token'ini dogrula. Gecerli ise user_id dondur."""
    try:
        payload = jwt.decode(
...
def _sha256_token(token: str) -> str:
    """Token'i SHA-256 ile hashle (DB depolama için)."""
...
def verify_refresh_token(token: str, db) -> str:
    ...
    veya replay saldirisi tespiti icin bu izler SIEM/alerting tarafindan islenmelidir.
    """
    from app.infra.models import RefreshToken
    ...
    try:
        ...
        raise ValueError(f"Geçersiz refresh token: {e}") from e

    if payload.get("type") != "refresh":
        logger.warning(
            "Refresh token dogrulama basarisiz — yanlis token tipi: sub=%s type=%s",
    ...
    if record.revoked:
        logger.warning(
            "Refresh token dogrulama basarisiz — token iptal edilmis: sub=%s jti=%s user_id=%s",
    ...
    if record.token_hash != _sha256_token(token):
        logger.warning(
            "Refresh token dogrulama basarisiz — hash eslesmedi (possible forgery): sub=%s jti=%s user_id=%s",
            payload.get("sub"), jti, record.user_id,
        )
