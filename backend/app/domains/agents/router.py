...
def get_heal_stats(user: CurrentUser, db: DB, project_id: str = ""):
    ...
    try:
        ...
        # Tüm heal kayitlarini al
        chunks = store.retrieve(
            "Selector Heal tamir selector",
            top_k=200,
            sources=["error_pattern"],
        )

        heal_chunks = [c for c in chunks if (c.metadata or {}).get("type") == "selector_heal"]
        ...
        stats.success_rate = 1.0  # Kayitlilar zaten başarılı olanlar
        ...
        pass  # KnowledgeStore erisilemezse bos stats don
