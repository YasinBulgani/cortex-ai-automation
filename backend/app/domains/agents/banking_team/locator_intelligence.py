...
Yetenekler:
  1. Stability Analysis — Tüm selector'lari tarayip kirilma riski hesapla
  2. Proactive Suggestions — Zayif selector'lar için iyilestirme öner
  3. Heal History Learning — Geçmiş heal'lerden pattern ogren
  4. POM Generator — DOM'dan Page Object Model üret
...
_STABILITY_RULES: list[tuple[str, int, str]] = [
    ...
    ("[name=", 3, "name attribute: form element'leri için iyi"),
...
_UNSTABLE_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"nth-child\(\d+\)"), "nth-child: DOM sırası degisince kirilir"),
...
class LocatorIntelligence(BaseAgent):
    ...
    def _generate_quick_improvements(self, weak_locators: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Zayif locator'lar için hizli iyilestirme onerileri (LLM gerektirmez)."""
    ...
    def suggest_improvements(
        ...
        """
        Zayif locator'lar için daha iyi alternatifler öner.
    ...
    def _llm_suggest_improvements(
        ...
        system = (
            "Sen bir Playwright test muhendisisin. Asagidaki zayif selector'lar için "
            "DOM'a bakarak daha stabil alternatifler öner.\n\n"
            "Oncelik sırası: data-testid > role > aria-label > id > name > placeholder > text\n\n"
    ...
    def learn_from_history(self, heal_history: list[dict[str, Any]]) -> dict[str, Any]:
        """
        Geçmiş heal kayitlarindan pattern ogren.
        ...
        if avg_conf < 0.70:
            recommendations.append(
                ...
                "LLM heal'lere guvenilirlik artirmak için DOM snippet kalitesini artirin." % avg_conf
            )
    ...
    def generate_page_object(
        ...
        """
        DOM element'lerinden TypeScript Page Object Model sinifi üret.
        ...
        system = (
            ...
            "Verilen sayfa element'lerinden TypeScript Page Object Model sinifi üret.\n\n"
            "Kurallar:\n"
            "1. Her element için en stabil selector'i sec: "
            "data-testid > role > aria-label > id > name > placeholder > text\n"
            "2. Getter property'ler kullan (get xxx())\n"
            "3. Interaktif element'ler için action method'lari ekle\n"
            ...
            "  get submitButton() { return this.page.getByRole('button', { name: 'Giriş' }); }\n\n"
            "  // Actions\n"
    ...
    def _fallback_generate_pom(
        ...
        """LLM başarısız olursa kural bazli POM üret."""
        lines: list[str] = [
            "import { Page } from '@playwright/test';",
            "",
    ...
    def _predict_single(
        ...
        """Tek bir locator için kirilma tahmini."""
    ...
    def run(self, context: dict[str, Any]) -> AgentResult:
        """
        Analiz pipeline'ini çalıştır.

        context keys:
          locators        — [{"selector": "...", "name": "...", "type": "..."}]
          dom_snippet     — DOM HTML (POM uretimi ve iyilestirme onerileri için)
          page_url        — Sayfa URL'si (POM için)
          page_name       — POM sinif adi
          heal_history    — Geçmiş heal kayitlari
...
def _url_to_class_name(url: str) -> str:
    """URL'den PascalCase sinif adi üret."""
    if not url:
        return "UnknownPage"
...
def _generate_testid_suggestion(name: str, selector: str) -> str:
    """Element adi veya selector'dan makul bir data-testid öner."""
    if name:
...
def _element_to_property_name(elem: dict[str, Any]) -> str:
    """DOM element'inden camelCase property adi üret."""
    # Prefer data-testid
...
def _best_selector_for_element(elem: dict[str, Any]) -> str:
    """Element için en iyi Playwright selector'i sec."""
    testid = elem.get("data_testid") or elem.get("data-testid", "")
    if testid:
