...
Zincir (oncelik sirasina gore):
  ...
  Strateji 3: Similarity Search — Kırık selector'dan hint cikar, benzer element bul
  ...
  Strateji 6: Playwright MCP Live — Gerçek browser'da DOM tarayarak bul

Her strateji bir confidence skoru doner (0.0-1.0).
Zincir, confidence >= threshold olan ilk başarılı stratejiyle durur.
Tüm stratejiler basarisizsa, en yuksek confidence'li sonucu doner.

Entegrasyon:
  TestRunner -> LocatorFallbackChain.resolve() -> Çalışan selector doner
...
def _not_found(strategy: str, reason: str = "") -> FallbackResult:
    """Hizli 'bulunamadi' sonucu oluştur."""
...
def _extract_hints_from_selector(selector: str) -> dict[str, str | None]:
    """
    Kırık selector'dan ipucu cikar: tag, id fragment, class, text, attribute.
    Strategy 3 (similarity) için kullanilir.
    """
...
def _build_pw_selector(attr: str, value: str, tag: str = "") -> str:
    """Attribute + value'dan Playwright selector string oluştur."""
    if attr == "data-testid":
...
class LocatorFallbackChain:
    ...
    def _strategy_similarity(self, selector: str, dom_snippet: str, **kwargs: Any) -> FallbackResult:
        """Kırık selector'dan hint cikar, DOM'da en benzer elementi bul."""
        if not dom_snippet:
    ...
    def _strategy_cache(self, selector: str, **kwargs: Any) -> FallbackResult:
        """KnowledgeStore'dan daha once ayni selector için heal var mi?"""
        try:
    ...
    def _strategy_llm(self, selector: str, dom_snippet: str, error_message: str, **kwargs: Any) -> FallbackResult:
        """LLM ile DOM + hata analizi yaparak yeni selector öner."""
        if not dom_snippet and not error_message:
            return _not_found("llm", "DOM ve hata mesajı yok — LLM'e gonderilecek veri yetersiz")
        ...
        try:
            ...
            return _not_found("llm", "LLM cagrisi başarısız: %s" % str(exc)[:100])
    ...
    def _strategy_playwright_live(
        ...
        """Playwright MCP ile gerçek browser'da DOM tarayarak element bul."""
        if not session_id:
