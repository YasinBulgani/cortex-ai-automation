...
Entegrasyonlar:
  - FewShotBank:      Her mod için bankacilik domain few-shot ornekleri
  - SmartModelRouter: Görev tipine ve endpoint ozelliklerine gore optimal model secimi
...
_SPEC_ANALYSIS_PROMPT = """Sen kidemli bir API Guvenligi ve Mimari Uzmanisin. Turkce yanıt ver.
...
_TEST_GENERATION_PROMPT = """Sen bankacilik sektorunde 15+ yil deneyime sahip kidemli bir API QA Muhendisisin. Turkce yanıt ver.

Verilen endpoint listesi için KAPSAMLI test senaryolari üret. Her endpoint için en az 5-8 test case oluştur.

TEST KATEGORILERI:
1. POSITIVE — Gecerli parametrelerle başarılı cagri (happy path)
2. NEGATIVE — Zorunlu alan eksik, geçersiz tip, bos deger, null
...
- Kimlik dogrulama: Brute-force (5 yanlis deneme sonrasi kilit), token süresi, refresh rotation
- IBAN dogrulama: Geçersiz format, uzunluk, checksum, farkli ulke IBAN'i
...
_SECURITY_AUDIT_PROMPT = """Sen OWASP API Security Top 10 ve bankacilik regülasyonlari konusunda uzman bir Siber Güvenlik Denetcisisin. Turkce yanıt ver.

Verilen API endpoint'leri için OWASP API Security Top 10 (2023) + bankacilik guvenligi testleri üret.
...
- API5:2023 Broken Function Level Authorization — Admin fonksiyonlarina normal kullanıcı erisimi
...
- KVKK: Kişisel veri minimizasyonu, maskeleme, erisim kaydi
  ...
  "security_tests": [
    {
      ...
      "attack_scenario": "Kullanıcı A, Kullanıcı B'nin account_id'sini kullanarak...",
...
_CHAIN_BUILDER_PROMPT = """Sen API entegrasyon ve test otomasyon uzmanisin. Turkce yanıt ver.

Verilen endpoint listesini analiz ederek anlamli API call chain'leri (is akislari) oluştur.
...
class ServiceTestAgent(BaseAgent):
    ...
    max_tokens = 8192  # Uzun test listeleri için
    ...
    def _extract_endpoint_keywords(endpoints: list) -> List[str]:
        """Endpoint listesinden few-shot eslestirme için anahtar kelimeler cikar."""
    ...
    def _build_context(self, ctx: dict) -> str:
        """Çalışma moduna gore LLM'e gonderilecek context'i oluştur.
        ...
        if mode == "test_generation":
            ...
            parts.append(f"Her endpoint için en fazla {max_per_endpoint} test üret.")
    ...
    def safe_run(self, ctx: dict) -> AgentResult:
        ...
        ctx parametreleri:
          ...
          test_types        : Istenilen test tipleri (test_generation için)
          max_tests_per_endpoint : Her endpoint için max test sayisi
          owasp_focus       : Odaklanilacak OWASP kategorileri (security_audit için)
          flow_hints        : Is akisi ipuclari (chain_builder için)
        ...
        # Context oluştur (few-shot ornekleri dahil)
        ...
        try:
            ...
            if result is None:
                return AgentResult(
                    ...
                    success=False,
                    error="LLM bos yanıt dondurdu",
                )

            # CrossAgentMemory'ye sonuç yayinla (fire-and-forget)
    ...
    def _publish_to_memory(self, mode: str, data: dict, endpoints: list) -> None:
        """CrossAgentMemory'ye üretim sonuclarini yayinla."""
        try:
