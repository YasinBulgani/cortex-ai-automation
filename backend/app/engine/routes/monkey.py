"""
Monkey Testing routes — Flask engine'den port edilmiş.

ÖNCE (Flask):
  /engine/routes/monkey_routes.py — Blueprint, port 5001

SONRA (FastAPI):
  /backend/app/engine/routes/monkey.py — APIRouter, port 8000 (consolidated)
...
router = APIRouter(prefix="/api/monkey-testing", tags=["engine", "monkey"])
...
ACTION_WEIGHTS = {
    "click": 25, "fill": 15, "scroll": 10, "navigate": 10, "hover": 8,
    "double_click": 5, "right_click": 3, "keyboard": 5, "tab_navigation": 4,
    "select_change": 5, "checkbox_toggle": 5, "resize_viewport": 3, "back_forward": 2,
}
...
FUZZ_STRINGS = [
    ...
    "<h1>Heading Injection</h1>", "https://evil.com/phishing",
...
def _categorize_network_error(status_code: int) -> str:
    if status_code == 400: return "Bad Request (400)"
    if status_code == 401: return "Unauthorized (401)"
    if status_code == 403: return "Forbidden (403)"
    if status_code == 404: return "Not Found (404)"
    if status_code == 405: return "Method Not Allowed (405)"
    if status_code == 408: return "Request Timeout (408)"
    if status_code == 429: return "Rate Limited (429)"
    if 400 <= status_code < 500: return f"Client Error ({status_code})"
    if status_code == 500: return "Internal Server Error (500)"
    if status_code == 502: return "Bad Gateway (502)"
    if status_code == 503: return "Service Unavailable (503)"
    if 500 <= status_code < 600: return f"Server Error ({status_code})"
...
def _sample_memory(page) -> Optional[dict]:
    try:
        data = page.evaluate("""
            () => {
...
def _compute_stability_score(error_count: int, action_errors: int, a11y_summary: dict, memory_leak: bool) -> int:
    score = 100
    score -= error_count * 3
...
def _generate_analysis(
    actions_log: list, console_errors: list, network_errors: list,
    pages_visited, perf_metrics: list, stability_score: int,
    a11y_data: Optional[dict] = None, memory_leak: bool = False,
) -> dict:
    scenarios: list[dict] = []
    bugs: list[dict] = []
    recommendations: list[dict] = []

    error_pages: set[str] = set()
    ...
    network_categories: dict[str, list] = {}
    for err in network_errors:
    ...
    if memory_leak:
        bugs.append({
            "category": "Bellek Sızıntısı (Memory Leak)", "severity": "critical", "count": 1,
            "sample": "JS Heap monoton artış tespit edildi (>%50 büyüme)", "affected_pages": [],
        })
    ...
    scenarios.append({
        ...
        "description": "Farklı ekran boyutlarında uygulamanın doğru görüntülenmesi.",
        "steps": [
    ...
    scenarios.append({
        ...
        "description": "Hızlı ve yoğun etkileşim altında uygulama davranışı.",
        "steps": [
    ...
    if any("Güvenlik" in b["category"] or b["category"] == "Accessibility (WCAG 2.0 A+AA)" for b in bugs):
        recommendations.append({"priority": "critical", "text": "Güvenlik ve/veya erişilebilirlik bulguları var."})
    if memory_leak:
        recommendations.append({"priority": "high", "text": "Bellek sızıntısı tespit edildi. useEffect cleanup ve event listener yönetimini gözden geçirin."})
    ...
    return {
        "scenarios": scenarios, "bugs": bugs, "recommendations": recommendations,
        "risk_level": risk_level,
        "summary": {
            "total_bugs": len(bugs), "critical_bugs": critical_count,
            "warning_bugs": warning_count, "scenarios_generated": len(scenarios),
...
def _do_click(page, record: dict) -> None:
    ...
    if visible:
        ...
        try:
            record["target"] = (chosen.inner_text() or "")[:60]
        except Exception:
            record["target"] = chosen.evaluate("e => e.tagName.toLowerCase()")
...
def _do_fill(page, record: dict) -> None:
    ...
    if visible:
        ...
        try:
            ...
            record["result"] = f"fill_error: {str(exc)[:80]}"
    else:
        record["result"] = "no_visible_inputs"


def _do_scroll(page, record: dict) -> None:
...
def _do_navigate(page, record: dict, pages_visited: set, perf_metrics: list, base_url: str) -> None:
    links = page.evaluate("""
...
def _do_double_click(page, record: dict) -> None:
    els = page.query_selector_all("td, div, span, p, li")
    visible = [e for e in els if _safe_visible(e)]
    if visible:
        random.choice(visible[:20]).dblclick(timeout=3000)
        record["result"] = "double_clicked"
    else:
        record["result"] = "no_elements"


def _do_right_click(page, record: dict) -> None:
    els = page.query_selector_all("*")
    visible = [e for e in els if _safe_visible(e)]
    if visible:
        random.choice(visible[:30]).click(button="right", timeout=3000)
...
def _do_keyboard(page, record: dict) -> None:
...
def _do_tab_navigation(page, record: dict) -> None:
...
def _do_select_change(page, record: dict) -> None:
    ...
    if visible:
        ...
        if options:
            ...
            record.update({"value": val[:60], "result": "option_selected"})
        else:
...
def _do_resize(page, context, record: dict) -> None:
    ...
    record.update({"viewport": f"{size['width']}x{size['height']} ({label})", "result": "resized"})
    page.wait_for_timeout(500)


def _do_back_forward(page, record: dict) -> None:
    ...
    try:
        ...
        record.update({"direction": action, "result": f"{action}_navigated"})
        page.wait_for_timeout(500)
    except Exception as exc:
        record["result"] = f"{action}_error: {str(exc)[:80]}"


def _build_dispatch(page, context, pages_visited, perf_metrics, url) -> dict:
    return {
...
def api_monkey_run_stream(body: MonkeyRunBody):
    """Monkey test çalıştırır ve SSE stream ile gerçek zamanlı ilerleme döner."""
    url = body.url.strip()
    if not url.startswith(("http://", "https://")):
        url = "https://" + url

    max_actions = min(body.max_actions, 200)
    enabled_actions = body.config.get("enabled_actions", list(ACTION_WEIGHTS.keys()))
    ...
    def generate() -> Iterator[str]:
        ...
        yield _sse("start", {
            "run_id": run_id, "url": url, "max_actions": max_actions,
            "record_video": body.record_video, "timestamp": datetime.now().isoformat(),
        })

        try:
            with sync_playwright() as p:
                ctx_kwargs: dict[str, Any] = {"viewport": {"width": 1280, "height": 800}, "locale": "tr-TR"}
                ...
                dispatch = _build_dispatch(page, context, pages_visited, perf_metrics, url)
                ...
                for i in range(max_actions):
                    ...
                    try:
                        ...
                        record["result"] = f"exception: {str(exc)[:120]}"
                    ...
                    try:
                        ...
                        if triggered_error:
                            ...
                            yield _sse("error_shot", {
                                "step": i + 1, "image": cur_b64,
                                "url": page.url, "visual_diff": vd,
                                "timestamp": datetime.now().isoformat(),
                            })
                        elif i % 10 == 0:
                            yield _sse("frame", {
                                "step": i + 1, "url": page.url, "image": cur_b64,
                                "visual_diff": vd, "timestamp": datetime.now().isoformat(),
                            })
                    except Exception:
                        pass
            ...
            yield _sse("fail", {"run_id": run_id, "error": str(exc), "trace": traceback.format_exc()[-2000:]})
...
@router.post("/run")
def api_monkey_run(body: MonkeyRunBody):
    """Monkey test çalıştırır, tamamlanınca JSON döner."""
    url = body.url.strip()
    if not url.startswith(("http://", "https://")):
        url = "https://" + url
