...
def _parse_table(table_text: str) -> list[list[str]]:
    """Parse a Gherkin data table string into a list of rows (each row is a list of cells)."""
    rows = []
...
@given(parsers.re(r"adımlar listesi hazırlanıyor:\n(?P<table>[\s\S]+)"))
def set_scenario_steps(ctx: dict, table: str):
    ...
    if not rows:
        ctx.setdefault("scenario_body", {})["steps"] = []
        return
...
@given(parsers.re(r"projede şu senaryolar mevcut:\n(?P<table>[\s\S]+)"))
def seed_scenarios(api: TestClient, ctx: dict, table: str):
    rows = _parse_table(table)
    headers = [h.lower() for h in rows[0]]
