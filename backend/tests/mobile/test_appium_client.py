...
# ═══════════════════════════════════════════════════════════════
# UNIT — Locator strategy map bütünlüğü
# ═══════════════════════════════════════════════════════════════
class TestLocatorStrategies:
    def test_core_strategies_present(self):
        assert LOCATOR_STRATEGIES["accessibilityId"] == "accessibility id"
        assert LOCATOR_STRATEGIES["xpath"] == "xpath"
        assert LOCATOR_STRATEGIES["id"] == "id"
        assert LOCATOR_STRATEGIES["className"] == "class name"

    def test_extended_strategies_present(self):
        assert LOCATOR_STRATEGIES["cssSelector"] == "css selector"
        assert LOCATOR_STRATEGIES["iosClassChain"] == "-ios class chain"
        assert LOCATOR_STRATEGIES["image"] == "image"
        assert LOCATOR_STRATEGIES["name"] == "name"

    def test_unknown_strategy_raises_on_find_element(self):
        def handler(_: httpx.Request) -> httpx.Response:
        ...
        c.session_id = "sid"
        with pytest.raises(AppiumError, match="locator stratejisi"):
            c.find_element("unknown_strategy", "x")
