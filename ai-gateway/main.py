...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        _mono = _pl.Path(__file__).resolve().parent.parent
        sys.path.insert(0, str(_mono / "packages/telemetry/src"))
        from neurex_telemetry.logging import configure_logging  # type: ignore
        configure_logging(service="neurex-ai-gateway")
    except Exception:
        pass
    try:
        import pathlib as _pl
        _mono = _pl.Path(__file__).resolve().parent.parent
        sys.path.insert(0, str(_mono / "backend"))
        from app.infra.telemetry import init_otel  # type: ignore
        init_otel(service_name="neurex-ai-gateway", instrument_fastapi=True, fastapi_app=app)
    except Exception:
        pass
    ...
    yield
    # shutdown
    logger.info("AI Gateway kapatiliyor...")
