...
const DESCRIPTIONS: Record<ServiceTestingStage, string> = {
  spec: "OpenAPI veya Swagger baglamini içe alin, riskli endpointleri secin ve AI ile test adaylari üretin.",
  chain: "Request'leri zincire baglayip veri akisini, extraction kurallarini ve assertion mantigini tasarlayin.",
  run: "Servis testlerini çalıştırin, sonuç raporlarini inceleyin ve bir sonraki kalite dongusunu belirleyin.",
};

const PRODUCT_NOTES: Record<ServiceTestingStage, Partial<Record<ProductFamilyId, string>>> = {
  spec: {
    one: "Platform cekirdegi için servis baglami ve entegrasyon riski burada gorunur hale gelir.",
    service: "Service lensinde bu asama ana giriş noktasi; spec, risk ve AI test adayi kalitesi burada belirlenir.",
    intelligence: "AI kalite katmani için grounding ve risk sinyali bu ekranda guclenir.",
  },
  chain: {
  ...
  run: {
    one: "Platform ritmi ve servis koşu gorunurlugu burada netlesir.",
    service: "Service odaginda gerçek kalite sinyali assertion sonuçlari ve run davranisindan gelir.",
    intelligence: "AI analizi ve kendini iyilestiren dongu için taze servis koşu verisi burada üretilir.",
  },
};
...
export function ServiceTestingGuide({
  ...
  const productNote =
    ...
    "Bu akis seçili urunu destekleyen genel servis kalite hattinin bir parcasi olarak çalışir.";
