...
name: Neurex QA Operations — Test Koşumu
...
jobs:
  visium-ops:
    ...
    steps:
      ...
      - name: Neurex testlerini tetikle
        run: |
          curl -s -X POST \\
            -H "Content-Type: application/json" \\
            -H "X-CI-Token: \${{ secrets.NEUREX_CI_TOKEN }}" \\
            -d '{
    ...
    <div className="rounded-2xl bg-slate-900 border border-slate-800 overflow-hidden">
      ...
      <div className="p-5 space-y-5">
        ...
        <div className="rounded-xl bg-slate-800/40 border border-slate-700/50 px-4 py-3 space-y-1.5">
          ...
          {[
            { key: "NEUREX_CI_TOKEN",    value: "Ayarlar → Engine Secret Key",        note: "ENGINE_SECRET_KEY ile aynı" },
            { key: "NEUREX_CI_BASE_URL", value: triggerUrl,                           note: "Trigger endpoint" },
          ].map(({ key, value, note }) => (
...
function EngineWebhookPanel() {
  ...
  useEffect(() => {
    fetch(`${ENGINE_BASE}/api/webhooks/config`).then(r => r.json()).then(setCfg).catch((err) => console.warn("[cicd]:", err));
    fetch(`${ENGINE_BASE}/api/webhooks/events?limit=10`).then(r => r.json()).then(d => setEvents(d.events ?? [])).catch((err) => console.warn("[cicd]:", err));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6 flex flex-col gap-4" data-testid="cicd-page">
      ...
      <FlowGuideCard
        projectId={projectId}
        stage="execute"
        title="CI/CD ve otomatik koşu akisi"
        ...
        nextLabel="Koşulari ac"
        nextHref={`/p/${projectId}/executions`}
        supportLinks={[
          { label: "Koşular", href: `/p/${projectId}/executions` },
