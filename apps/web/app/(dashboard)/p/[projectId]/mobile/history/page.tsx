    ...
    <div className="min-h-screen bg-slate-950 p-6" data-testid="mobile-history-page">
      ...
      <div className="grid grid-cols-3 gap-3 my-5">
        <div className="rounded-xl border border-slate-700 bg-slate-900/40 px-4 py-3 flex items-center justify-between">
          <span className="text-xs text-slate-400">Toplam Koşum</span>
        ...
        <div className="rounded-xl border border-slate-700 bg-slate-900/40 overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {groupByDevice ? (
                (() => {
                  // device_name'e göre grupla
                  const groups = runs.reduce<Record<string, MobileRun[]>>((acc, r) => {
                  ...
                  return Object.entries(groups).map(([deviceName, groupRuns]) => (
                    <>
                      <tr key={`group-${deviceName}`} className="bg-slate-800/60">
                        <td colSpan={5} className="px-4 py-2 text-xs font-bold text-slate-300">
                          📱 {deviceName || "Cihaz Bilinmiyor"}
                        </td>
                      </tr>
                      {groupRuns.map(r => {
