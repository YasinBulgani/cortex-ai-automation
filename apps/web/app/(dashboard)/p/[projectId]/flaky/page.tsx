...
type AnomalyIssue = "timeout" | "slow" | "flapping" | "error";
type AnomalyReport = {
  total_tested: number;
  anomaly_count: number;
  avg_duration_ms: number;
  anomalies: Array<{ testId: string; issues: AnomalyIssue[] }>;
};

type TabId = "list" | "api-flaky" | "quarantine" | "anomaly";

const REC_COLORS: Record<string, string> = {
  quarantine: "bg-red-500/10 border-red-500/20 text-red-400",
  investigate: "bg-amber-500/10 border-amber-500/20 text-amber-400",
...
const ISSUE_COLORS: Record<string, string> = {
  ...
  slow: "bg-amber-500/10 border-amber-500/20 text-amber-400",
  flapping: "bg-violet-500/10 border-violet-500/20 text-violet-400",
  error: "bg-red-500/10 border-red-500/20 text-red-400",
};

const TABS: { id: TabId; label: string }[] = [
  { id: "list", label: "Flaky Listesi" },
  { id: "api-flaky", label: "API Flaky" },
  { id: "quarantine", label: "Karantina" },
      ...
      {activeTab === "anomaly" && (
        anomalyReport ? (
          ...
          <div className="p-8">
            <EmptyState
              icon="🔬"
              title="Anomali analizi yapılmadı"
              description="Yukarıdaki 'AI Anomali Analizi' butonuna tıklayarak analizi başlatın"
            />
          </div>
        )
      )}
