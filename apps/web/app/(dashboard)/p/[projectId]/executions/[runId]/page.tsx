  ...
  return (
    <div className="flex h-full flex-col overflow-hidden" data-testid="execution-detail-page">
      ...
      <div className="border-b border-slate-800 px-6 py-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <ExportMenu projectId={projectId} runId={runId} />
            <RerunFailedButton projectId={projectId} runId={runId} hasFailed={(data.results || []).some((r: any) => r.status === "failed")} />
            <button
              onClick={completeRun}
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        {displayMetrics.failed > 0 && (
          <div
            data-testid="ai-debug-banner"
            className="flex items-center justify-between rounded-xl border border-amber-700/40 bg-amber-950/20 px-4 py-3"
          >
            <div className="flex items-center gap-3">
              <span className="text-2xl">🧠</span>
              <div>
                <p className="text-sm font-semibold text-amber-200">
                  AI Debug Raporu Hazır
                </p>
                <p className="text-xs text-amber-300/80">
                  {displayMetrics.failed} başarısız senaryo için kök neden analizi + düzeltme önerileri
                </p>
              </div>
            </div>
            <Link
              href={`/p/${projectId}/debug-report?runId=${runId}`}
              data-testid="ai-debug-link"
