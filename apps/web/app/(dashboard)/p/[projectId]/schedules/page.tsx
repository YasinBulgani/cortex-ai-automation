...
type MobileForm = { name: string; cron_expression: string; platform: string; device_name: string; scenario_ids: string };
const emptyMobileForm: MobileForm = { name: "", cron_expression: "", platform: "ios", device_name: "", scenario_ids: "" };

type EngineSchedule = { id: string; name: string; cron: string; cron_expression?: string | null; browser?: string | null; is_active: boolean; last_run?: string | null; last_run_at?: string | null; next_run?: string | null; run_count?: number | null };

const MOBILE_DEVICES: Record<string, string[]> = {
  ios: ["iPhone 15 Pro", "iPhone 14", "iPhone 13", "iPad Pro"],
  android: ["Pixel 8", "Samsung S24", "OnePlus 12", "Galaxy Tab S9"],
};
...
/* ── Neurex Farm Mobile Scheduler Panel ───────────────────────────────────── */
function MobileSchedulePanel({
  ...
  return (
    <SectionCard
      title="📱 Neurex Farm Zamanlayıcıları"
      icon={
        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function EngineSchedulerPanel({ projectId }: { projectId: string }) {
  ...
  async function create(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const createRes = await fetch(`${ENGINE_BASE}/api/schedules`, {
        method: "POST",
  ...
  async function toggle(s: EngineSchedule) {
    const toggleRes = await fetch(`${ENGINE_BASE}/api/schedules/${s.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  async function triggerNow(id: string) {
    setTriggering(id);
    await fetch(`${ENGINE_BASE}/api/schedules/${id}/trigger`, { method: "POST" })
  ...
  async function del(id: string) {
    const delRes = await fetch(`${ENGINE_BASE}/api/schedules/${id}`, { method: "DELETE" });
          ...
          <tbody>
            {schedules.map(s => (
              <tr key={s.id} className="border-b border-slate-800 last:border-0 hover:bg-slate-800/30 group">
                ...
                <td className="px-4 py-3 text-xs text-slate-400">{fmtDate(s.last_run_at ?? null)}</td>
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6 flex flex-col gap-4" data-testid="schedules-page">
      ...
      <MetricRow cols={4}>
        ...
        <StatCard label="Neurex Farm" value={mobileSchedules.length} color="violet" sub="Mobile" />
