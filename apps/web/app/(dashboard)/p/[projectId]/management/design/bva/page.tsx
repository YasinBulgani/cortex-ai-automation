...
export default function ManagementBvaPage({ params }: { params: { projectId: string } }) {
  const createRun = useCreateBvaRun();
  ...
  const submit = async (mode: "ai" | "deterministic") => {
    setPromotedIds([]);
    setSelected(new Set());
    const payload = {
      ...
      fields: fields.map((f) => ({
        ...f,
        // When user selects "deterministic" we still call the same endpoint;
        // the backend falls back to the deterministic generator whenever the
        // AI gateway is unavailable. We hint with a dummy requirement_text so
        // observability can split the two intents.
        min_value: f.min_value || null,
        max_value: f.max_value || null,
      })),
      requirement_text:
        mode === "deterministic" ? "[deterministic-fallback-requested]" : requirementText,
    };
  ...
  return (
    <ManagementShell
      ...
      description="Sınır değer analizi (min / min+1 / nominal / max-1 / max / invalid) ile case taslakları üret; AI veya deterministik fallback kullan."
      active="management/design/bva"
    >
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <ManagementStat
          label="Generated"
          value={String(run?.generated_cases.length ?? 0)}
          note={run ? `source: ${run.source}` : "henüz çalıştırılmadı"}
        />
        <ManagementStat
          label="Promoted"
          value={String(promotedIds.length)}
          note="bu oturumda TestCase'e dönüştürülen"
        />
      </div>
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
        <ManagementPanel title="Field Specs">
          <div className="space-y-3">
            ...
            <textarea
              ...
              rows={3}
              placeholder="Sistemin kabul ettiği değer aralıklarını ve davranışı açıklayın..."
              ...
              >
                <div className="grid gap-2 md:grid-cols-[1fr_auto_auto]">
                  <input
                    ...
                    placeholder="field name (örn. age)"
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              >
                {createRun.isPending ? "Üretiliyor..." : "AI ile üret"}
              </button>
              <button
                type="button"
                disabled={!canSubmit || createRun.isPending}
                onClick={() => submit("deterministic")}
        ...
        <ManagementPanel title={run ? `Generated cases (${run.generated_cases.length})` : "Generated cases"}>
          {!run && (
            <p className="text-sm text-slate-500">Henüz üretim çalıştırılmadı.</p>
          )}
          {run && run.generated_cases.length === 0 && (
            <p className="text-sm text-slate-500">Bu spec için case üretilemedi.</p>
          )}
          {run && run.generated_cases.length > 0 && (
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <input
                  ...
                  placeholder="suite_id (opsiyonel)"
