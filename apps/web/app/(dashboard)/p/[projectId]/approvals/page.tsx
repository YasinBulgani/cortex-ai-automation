...
function mapStatusToColumn(_status: string, decision: string | null): ColumnId {
  if (decision === "approved" || _status === "approved") return "approved";
  if (decision === "rejected" || _status === "rejected") return "rejected";
  return "pending";
}
...
function SortableCard({
  ...
  return (
    ...
    >
      ...
      <p className="mb-1 text-xs font-semibold text-white">{approval.title || "Onay kaydı"}</p>
      <p className="text-xs text-slate-300 line-clamp-3 leading-relaxed">
        {approval.source_text?.slice(0, 120) || "Kaynak metin yok"}
        {(approval.source_text?.length ?? 0) > 120 ? "..." : ""}
      </p>
      <div className="mt-2 flex flex-wrap gap-1">
        {approval.source_batch_id && (
          <span className="rounded bg-sky-500/10 px-1.5 py-0.5 text-[10px] text-sky-300">batch</span>
        )}
        {approval.source_test_case_id && (
          <span className="rounded bg-violet-500/10 px-1.5 py-0.5 text-[10px] text-violet-300">test-case</span>
        )}
        {approval.scenario_id && (
          <span className="rounded bg-emerald-500/10 px-1.5 py-0.5 text-[10px] text-emerald-300">scenario</span>
        )}
      </div>
    </div>
  );
...
export default function ApprovalsPage() {
  ...
  async function batchApprove() {
    const pending = rows.filter(r => mapStatusToColumn(r.status, (r.decision_trace?.decision as string | null) ?? null) === "pending");
  ...
  function findColumnForItem(itemId: string): ColumnId | undefined {
    ...
    return mapStatusToColumn(item.status, (item.decision_trace?.decision as string | null) ?? null);
  }
  ...
  return (
    <div className="flex min-h-screen flex-col gap-4 bg-slate-950 p-6" data-testid="approvals-page">
      ...
      {/* Stats */}
      <MetricRow cols={3}>
        <div data-testid="pending-count"><StatCard label="Bekleyen" value={pendingCount} color={pendingCount > 0 ? "amber" : "slate"} /></div>
      ...
      {/* Detail panel */}
      {active && (
        <div data-testid="split-view" className="rounded-xl border border-slate-700 bg-slate-900/40 p-4">
          <div className="grid gap-4 md:grid-cols-2 mb-4">
            ...
            <div data-testid="draft-panel">
              ...
              <pre data-testid="draft-editor" className="text-xs text-slate-300 overflow-auto max-h-40 bg-slate-950 rounded-lg p-3 border border-slate-800">
          ...
          <div className="mb-4 grid gap-4 md:grid-cols-3">
            <div className="rounded-lg border border-slate-800 bg-slate-950/60 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">Batch Ref</p>
              <p className="mt-1 font-mono text-xs text-slate-300">{active.source_batch_id?.slice(0, 12) || "—"}</p>
            </div>
            <div className="rounded-lg border border-slate-800 bg-slate-950/60 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">Test Case Ref</p>
              <p className="mt-1 font-mono text-xs text-slate-300">{active.source_test_case_id?.slice(0, 12) || "—"}</p>
            </div>
            <div className="rounded-lg border border-slate-800 bg-slate-950/60 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">Scenario Ref</p>
              <p className="mt-1 font-mono text-xs text-slate-300">{active.scenario_id?.slice(0, 12) || "—"}</p>
            </div>
          </div>

          {active.decision_note && (
            <div className="mb-4 rounded-lg border border-slate-800 bg-slate-950/60 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">Decision Note</p>
              <p className="mt-2 text-xs text-slate-300">{active.decision_note}</p>
            </div>
          )}

          {active.decision_trace && Object.keys(active.decision_trace).length > 0 && (
            <div className="mb-4 rounded-lg border border-slate-800 bg-slate-950/60 p-3">
              <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">Decision Trace</p>
              <pre className="mt-2 overflow-auto text-xs text-slate-300">
                {JSON.stringify(active.decision_trace, null, 2)}
              </pre>
            </div>
          )}

          {active.status === "pending" && !(active.decision_trace?.decision) && (
