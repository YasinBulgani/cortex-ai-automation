      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {teams.map((t) => (
          ...
          >
            ...
            <div className="text-xs text-slate-400 mt-2">{t.member_count} uye</div>
          </button>
        ))}
        {teams.length === 0 && (
          <div className="col-span-3 text-center text-sm text-slate-500 py-8">
            Henuz ekip yok. Yukaridaki formdan olusturabilirsiniz.
          </div>
        )}
      </div>
      ...
      {selectedTeam && (
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
          ...
          <div className="rounded-lg border border-slate-800 divide-y divide-slate-800">
            ...
            {members.length === 0 && (
              <div className="px-3 py-4 text-xs text-slate-500 text-center">Bu ekipte uye yok.</div>
            )}
          </div>
        </div>
      )}
