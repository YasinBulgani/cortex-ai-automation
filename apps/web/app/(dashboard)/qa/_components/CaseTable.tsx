    ...
    <div>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <input
          ...
          className="flex-1 min-w-[240px] rounded border border-border bg-surface-raised px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200"
        />
        <select value={suiteFilter} onChange={(e) => setSuiteFilter(e.target.value)} className="rounded border border-border bg-surface-raised px-2 py-1.5 text-sm">
          <option value="">Tüm suite</option>
          {suites.map((s) => (
            <option key={s} value={s}>{s}</option>
          ))}
        </select>
        <select value={prioFilter} onChange={(e) => setPrioFilter(e.target.value)} className="rounded border border-border bg-surface-raised px-2 py-1.5 text-sm">
          <option value="">Tüm öncelik</option>
          {["P0", "P1", "P2", "P3"].map((p) => <option key={p} value={p}>{p}</option>)}
        </select>
        <select value={autoFilter} onChange={(e) => setAutoFilter(e.target.value)} className="rounded border border-border bg-surface-raised px-2 py-1.5 text-sm">
        ...
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="rounded border border-border bg-surface-raised px-2 py-1.5 text-sm">
          ...
          <tbody>
            {filtered.slice(0, 250).map((tc) => (
              ...
              >
                ...
                <td className="px-3 py-1.5">
                  <span className={`rounded px-1.5 py-0.5 text-xs ${AUTO_TONE[tc.automation_status] || "bg-surface-overlay text-fg-muted"}`}>
                ...
                <td className="px-3 py-1.5">
                  {tc.last_status ? (
                    <span className={`rounded px-1.5 py-0.5 text-xs ${STATUS_TONE[tc.last_status] || "bg-surface-overlay"}`}>
                      {tc.last_status}
                    </span>
                  ) : (
...
function CaseDetailModal({ tc, onClose }: { tc: TestCase; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 p-4 pt-12" onClick={onClose}>
      <div className="max-h-[85vh] w-full max-w-4xl overflow-y-auto rounded-lg bg-surface-raised p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-start justify-between border-b border-border pb-3">
          ...
          <button onClick={onClose} className="rounded px-3 py-1 text-sm hover:bg-surface-overlay">
            ✕
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            {detail.automation?.refs?.length > 0 && (
              <div>
                <div className="mb-1 text-xs font-semibold uppercase text-fg-subtle">Automation refs</div>
            ...
            {detail.requirements?.length > 0 && (
              <div>
                <div className="mb-1 text-xs font-semibold uppercase text-fg-subtle">Requirements</div>
            ...
            {detail.pre_conditions?.length > 0 && (
              <div>
                <div className="mb-1 text-xs font-semibold uppercase text-fg-subtle">Pre-conditions</div>
                <div className="flex flex-wrap gap-1">
            ...
            <div>
              <div className="mb-2 text-xs font-semibold uppercase text-fg-subtle">Body</div>
