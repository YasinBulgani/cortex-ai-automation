...
export function useCreateEnvironment(projectId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: { name: string; description?: string; variables: Record<string, string>; sensitive_keys?: string[]; is_default?: boolean }) =>
...
export interface ChainRunResult {
  ...
  steps: { step: number; method: string; path: string; status_code: number; ok: boolean; duration_ms: number; error?: string }[];
  total_duration_ms: number;
  passed: number;
  failed: number;
}

export function useRunChain(projectId: string) {
  return useMutation({
    mutationFn: (chainId: string) =>
      apiFetch<ChainRunResult>(`${BASE(projectId)}/chains/${chainId}/run`, { method: "POST" }),
  });
}
...
export function useHealHistory(projectId: string | undefined, limit = 20) {
  return useQuery({
    queryKey: ["api-testing", projectId, "heal-history", limit],
    queryFn: () =>
      apiFetch<HealHistoryEntry[]>(
        `/api/v1/agents/heal/history?project_id=${projectId}&limit=${limit}`,
      ),
    enabled: !!projectId,
    staleTime: 60_000,
    retry: 1,
...
export function useManualHeal(projectId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: { run_id: string; test_case_ids?: string[] }) =>
      apiFetch<{ healed: boolean; message: string }>(
        `/api/v1/agents/heal/run`,
        { method: "POST", body: JSON.stringify({ project_id: projectId, ...body }) },
      ),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["api-testing", projectId] });
    },
