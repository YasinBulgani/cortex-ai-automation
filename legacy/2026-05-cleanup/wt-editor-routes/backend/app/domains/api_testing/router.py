...
def create_environment(
    ...
    """Yeni ortam degiskeni seti olustur."""
    env = ApiEnvironment(
    ...
    # is_default ise diger default'lari kaldir
    if body.is_default:
...
def list_environments(
    ...
    return db.query(ApiEnvironment).filter(
        ApiEnvironment.project_id == project_id,
    ).order_by(ApiEnvironment.created_at).all()
...
def list_endpoints(
    ...
    return q.order_by(ApiEndpoint.path).all()
...
async def execute_single_request(
    ...
    """
    Tek bir HTTP istegi calistir (Postman-style).
...
def get_execution_detail(
    ...
    """Tek bir run'in tam detayi — tum assertion sonuclari dahil."""
