"""
Execution Feedback Loop — Test calisma sonuclarindan ogrenme ve zenginlestirme.

Bu modul her test kosusu sonrasinda:
  1. Basarisiz testlerden hata paternleri cikarir → KnowledgeStore'a kaydeder
  2. Yavas yanit veren endpoint'leri tespit eder → performans uyarisi uretir
...
def learn_from_execution(
    ...
    Returns:
        Ozet istatistikler: {error_patterns, performance_warnings, recurring_failures, total_ingested}
    """
    try:
        store = KnowledgeStore(project_id=project_id)
    except Exception as exc:
        logger.warning("KnowledgeStore baglantisi kurulamadi: %s", exc)
        return {"error": str(exc), "total_ingested": 0}

    # Tum execution detail'leri yukle
    details = (
    ...
    if not details:
        logger.info("Run %s icin execution detail bulunamadi", run_id)
        return {"error_patterns": 0, "performance_warnings": 0,
                "recurring_failures": 0, "total_ingested": 0}

    # Test case bilgilerini toplu yukle
    ...
    # ── Basarisiz testlerden hata paternleri ──────────────────────────
    ...
    # ── Yavas yanitlar icin performans uyarilari ──────────────────────
...
def _build_failure_learning_text(
    ...
    """Basarisiz bir test icin yapilandirilmis ogrenme metni olustur."""
    tc = test_cases_map.get(detail.test_case_id) if detail.test_case_id else None
    ...
    # Beklenen vs gercek status code
...
def _guess_root_cause(detail: ApiExecutionDetail) -> Optional[str]:
    """Basarisiz test icin olasi root cause tahmini."""
...
def _detect_and_store_recurring_failures(
    ...
    """
    Tekrarlayan hatalari tespit et: Bir endpoint icin fail_count >= RECURRING_FAILURE_THRESHOLD
    ...
    for endpoint_key, failures in endpoint_failures.items():
        # Bu endpoint icin mevcut run'daki failure sayisi
...
def _build_stats_summary(
    ...
    """Endpoint istatistiklerinden ozet metin olustur."""
...
def get_learning_context(
    ...
    Args:
        ...
        mode: Calisma modu — "test_generation" hata paternleri alir,
...
def enrich_generation_prompt(
    ...
    """
    Test uretimi oncesi AI agent'a verilecek zenginlestirilmis baglam olustur.

    Args:
        project_id: Proje ID'si
        endpoints: Endpoint dict listesi ([{method, path, ...}])
        mode: Calisma modu (test_generation, security_audit, vb.)
        db: SQLAlchemy session (mevcut test sayisi hesabi icin)
