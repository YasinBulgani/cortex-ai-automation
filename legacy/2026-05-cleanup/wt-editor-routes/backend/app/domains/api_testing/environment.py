...
Desteklenen syntax'lar:
  ...
  {{env.variable_name}}       — Acik env referansi (ayni sey)
...
def _random_iban() -> str:
    """Rastgele gecerli formatta TR IBAN uret (dogrulama yok)."""
...
def resolve_string(
    ...
    Returns:
        Tum degiskenler cozulmus string
    """
    def _replacer(match: re.Match) -> str:
        key = match.group(1).strip()

        # env. prefix'ini kaldir
...
def resolve_dict(
    ...
    """
    Dict/list/string icindeki tum {{variable}} referanslarini recursive coz.
    """
    if isinstance(data, str):
        resolved = resolve_string(data, variables)
        # Eger tum string bir degisken ise ve deger non-string ise tipi koru
