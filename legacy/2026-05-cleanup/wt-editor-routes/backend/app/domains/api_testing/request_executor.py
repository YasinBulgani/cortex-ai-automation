...
class ExecutionResult:
    ...
    # Extracted variables (sonraki chain adimlari icin)
...
def _resolve_json_path_simple(data: Any, path: str) -> Any:
    """Basit JSON path cozumleyici (extraction icin)."""
...
async def execute_request(
    *,
    ...
    Args:
        ...
        variables: {{variable}} cozumleme icin degisken mapping'i
        assertions: Degerlendirilecek assertion listesi
        expected_schema: Contract testing icin beklenen JSON Schema
    ...
    try:
        ...
        result.error = f"Baglanti hatasi: {exc}"
        result.timing.total_ms = (time.monotonic() - t_start) * 1000
        return result
    ...
    result.success = True  # HTTP istegi basarili (assertion sonucu degil)

    # 4. Variable extraction (chain icin)
    if extract_rules:
...
async def execute_collection(
    ...
    *,
    ...
    """
    Bir koleksiyondaki tum istekleri sirasiyla calistir.
    ...
    Args:
        ...
        timeout: Her istek icin timeout
    ...
    for req in requests:
        ...
        # Extracted variables'i sonraki istekler icin ekle
