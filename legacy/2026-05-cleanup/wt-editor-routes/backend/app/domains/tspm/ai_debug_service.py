...
1. Kök nedeni sınıflandır (PRODUCT_BUG / TEST_ISSUE / ENVIRONMENT / AUTOMATION_DEBT)
...
MUTLAKA şu JSON formatında yanıt ver:
{
  "analyses": [
    {
      "test_id": "string",
      "root_cause_category": "PRODUCT_BUG|TEST_ISSUE|ENVIRONMENT|AUTOMATION_DEBT",
...
def _rule_based_analysis(
    ...
    for r in failed_results:
        ...
        rc = RootCauseAnalyzer.classify(error_msg, error_type)

        fix_steps = _default_fix_steps(rc.category.value, rc.subcategory)
        analyses.append({
            "test_id": r.get("test_id", "?"),
            "root_cause_category": rc.category.value,
            "root_cause_subcategory": rc.subcategory,
            "confidence": 0.6,
            "fix_steps": fix_steps,
