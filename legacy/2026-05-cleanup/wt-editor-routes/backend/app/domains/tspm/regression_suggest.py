...
def suggest_regression_sets(
    ...
    Öncelik sırası:
      ...
      2. OpenAI (geriye dönük uyumluluk, OPENAI_API_KEY varsa)
      3. Akıllı dummy gruplama (fallback)
    """
    if not scenarios:
        return []
    ...
    # Try OpenAI as legacy fallback (cached client)
    try:
        from app.core.config import settings as backend_settings  # type: ignore[import]
        if hasattr(backend_settings, "openai_api_key") and backend_settings.openai_api_key:
            try:
                from app.domains.ai.service import _get_openai_client
                client = _get_openai_client()
            except ImportError:
                from openai import OpenAI
                client = OpenAI(api_key=backend_settings.openai_api_key)
            full_content = user_content
            if extra_instructions:
                full_content += f"\n\nEk Talimatlar:\n{extra_instructions}"
            response = client.chat.completions.create(
                model=getattr(backend_settings, "openai_model", "gpt-4o-mini"),
                messages=[
            ...
            for s in sets:
                s["scenario_ids"] = [sid for sid in s.get("scenario_ids", []) if sid in valid_ids]
            logger.info(f"OpenAI regresyon önerisi: {len(sets)} set")
            return sets
    except Exception as e:
        logger.warning(f"OpenAI fallback hatası: {e}")
