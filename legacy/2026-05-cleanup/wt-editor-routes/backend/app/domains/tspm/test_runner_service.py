...
def _post_to_engine_with_retry(url: str, payload: dict) -> httpx.Response:
    ...
    resp = httpx.post(url, json=payload, headers=_IKEY, timeout=15.0)
...
def launch_execution_run(
    ...
    thread = threading.Thread(
        target=_run_worker,
        args=(run_id, execution_id, project_id, browser, tags or [], base_url, q),
        daemon=True,
    )
    thread.start()
    return run_id
    ...
    try:
        ...
        try:
            engine_resp = _post_to_engine_with_retry(
                url=f"{ENGINE_BASE}/api/nexus/run",
                payload={
            ...
            with httpx.stream(
                "GET",
                f"{ENGINE_BASE}/api/run/{engine_run_id}/stream",
                headers=_IKEY,
                timeout=300.0,
            ) as stream_resp:
            ...
            q.put({
                "type": "info",
                "text": f"⚠️ Engine gerçek koşumu başlatamadı ({exc}) — simülasyon modu",
            })
    ...
    try:
        ...
        try:
            engine_resp = httpx.post(
                f"{ENGINE_BASE}/api/mobile/run-parallel",
                json=payload,
                headers=_IKEY,
                timeout=10.0,
            )
            ...
            with httpx.stream(
                "GET",
                f"{ENGINE_BASE}/api/mobile/run/{engine_run_id}/stream",
                headers=_IKEY,
                timeout=600.0,
            ) as stream_resp:
                stream_resp.raise_for_status()
