...
def _get_conn():
    """
    psycopg2 baglantisi — connection pool ile yonetilir.
    Pool'da bos baglanti varsa onu kullan, yoksa yeni olustur.
    """
    import psycopg2
                ...
                try:
                    cur.execute(
                        ...
                        (
                            ...
                            record.system_prompt[:_SYSTEM_PREVIEW_LEN],
                            record.user_prompt[:_USER_PREVIEW_LEN],
                            record.response[:_RESPONSE_PREVIEW_LEN],
                            len(record.response),
                            record.temperature,
                            record.max_tokens,
                            record.latency_ms,
                    ...
                    cur.execute(
                        """
                        INSERT INTO llm_traces (
                            project_id, user_id, run_id, agent_name, model, phase,
                            system_prompt_preview, user_prompt_preview,
                            response_preview, full_response_length,
                            temperature, max_tokens, latency_ms,
                            success, error_message, json_parse_ok,
...
def update_json_parse_status(trace_id: int | None, success: bool) -> bool:
    """Belirli bir trace satirinin json_parse_ok alanini guncelle."""
    if not trace_id:
        return False
    try:
        conn = _get_conn()
...
def _empty_trace_stats(error: str | None = None) -> dict[str, Any]:
    payload = {
        "total_calls": 0,
        "successful": 0,
        "failed": 0,
...
def get_model_task_performance_stats(
    model: str,
    task_type: str,
    *,
    phase: str | None = None,
    ...
    normalized_phase = _normalize_phase(phase, normalized_task_type, is_streaming=False) if phase else None
    try:
        conn = _get_conn()
        try:
            with conn.cursor() as cur:
