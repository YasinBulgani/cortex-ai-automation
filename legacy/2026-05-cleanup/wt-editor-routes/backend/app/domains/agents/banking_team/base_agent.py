"""BaseAgent — tum banking ekibi ajanlarinin ortak tabani."""
    ...
    if _anthropic_agent_client is None:
        with _anthropic_lock:
            if _anthropic_agent_client is None:
                ...
                _anthropic_agent_client = Anthropic(api_key=settings.anthropic_api_key)
...
class BaseAgent:
    """
    Tum banking QA ajanlarinin temel sinifi.
    ...
    def call(
        ...
        """LLM'e istek gonder, ham yaniti dondur. Trace kaydeder.

        Fallback zinciri + exponential backoff ile retry:
          Model 1: retry 1 → retry 2 → FAIL → fallback
          Model 2: retry 1 → retry 2 → FAIL → fallback
          ...
        Tum modeller basarisiz olursa son hatayi raise eder.
        Her cagri (basarili/basarisiz) llm_traces tablosuna kaydedilir.
        """
        effective_provider = _resolve_effective_provider()
        ...
        # JSON mode icin user mesajina ek talimat (Ollama'da)
        ...
        for model_idx, current_model in enumerate(model_chain):
            ...
            # ── kwargs'i her model icin yeniden hazirla ──────────────────
            kwargs: dict[str, Any] = {
                "model": current_model,
                "messages": messages,
            ...
            # Ollama icin num_ctx — model bazli context window
            if effective_provider == "ollama":
            ...
            # Bu model tamamen basarisiz — fallback'e gec
            ...
            if next_model_idx < len(model_chain):
                ...
                logger.error(
                    "%s tum modeller basarisiz (%s): %s",
                    self.name,
                    " → ".join(model_chain),
                    last_error,
                )

        # Basarisiz cagriyi da trace'e kaydet
        latency_ms = int((time.time() - t0) * 1000)
        try:
            from app.domains.ai.llm_trace import log_llm_call
            self._last_trace_id = log_llm_call(
    ...
    def _extract_json_object(text: str) -> dict | None:
        ...
        # Tum { pozisyonlarini bul, en distandakini dene
        start = text.find("{")
        if start == -1:
            return None
    ...
    async def async_run(self, context: dict) -> AgentResult:
        ...
        # Default: sync run'i thread pool'da calistir
        import asyncio
