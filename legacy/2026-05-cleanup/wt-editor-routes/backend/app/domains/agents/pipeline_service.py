...
Akis:
  1. Proje Kurulum    -> TSPM'de proje olustur/bul
  2. Kesif            -> Engine crawl + ProjectScanner
  ...
  7. Test Kosumu       -> Engine uzerinden test calistir
...
@dataclass
class PipelineState:
    """Pipeline calisma durumu — thread-safe."""
...
class PipelineService:
    ...
    async def run(self, config: PipelineConfig) -> dict[str, Any]:
        """
        Tam pipeline calistir — analizden otomasyona.
        ...
        try:
            ...
            # Kurtarilabilir hata ise kismi sonuc don
        ...
        for attempt in range(1, config.max_quality_retries + 1):
            try:
                ...
                if result.success and result.data:
                    ...
                    # Feedback loop: QualityJudge geri bildirimi ile kodu yeniden uret
                    ...
                    self._log("warn", "QualityJudge sonuc dondurmedi, geciriliyor")
                    return True
            except Exception as exc:
    ...
    async def _regenerate_code_with_feedback(
        ...
        """Quality gate feedback ile kodu yeniden uret."""
        try:
            from app.domains.agents.banking_team.code_generator import CodeGeneratorAgent
            generator = CodeGeneratorAgent()
    ...
    async def _phase_auto_healing(self, test_results: dict, config: PipelineConfig) -> None:
        """Faz 10: Kirik testleri onar ve tekrar kos."""
        failed_count = test_results.get("total_failed", 0)
        self._log("info", f"Auto-healing basliyor: {failed_count} basarisiz test")
    ...
    async def _phase_report(self, config: PipelineConfig) -> dict:
        ...
        self._log("success", f"Rapor hazir — {len(self._state.scenarios)} senaryo, "
