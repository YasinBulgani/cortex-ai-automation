"""
CoverUp Test Generator — Kapsanmayan kod icin AI destekli test uretimi.

Akis:
  1. Coverage gap'lerini al (GapDetector'dan)
  2. Her gap icin kaynak kodu oku (varsa)
  3. LLM'e gonder: "Bu kapsanmayan kod icin test yaz"
  4. Framework'e uygun test kodu uret (Playwright/pytest/Jest/Vitest)
...
Kapsanmayan UI kodu icin Playwright TypeScript testleri yaz.

## Kurallar
- TypeScript kullan, async/await pattern
- Page Object Model (POM) tercih et
- data-testid > role > aria-label > text sirasinda secici kullan
...
Kapsanmayan Python API kodu icin pytest testleri yaz.
...
Kapsanmayan JS/TS kodu icin Jest veya Vitest testleri yaz.
...
- Turkce yorum satirlari, Ingilizce kod
- Async islemler icin async/await + resolves/rejects
...
class CoverUpTestGenerator(BaseAgent):
    """Kapsanmayan kod icin AI destekli test uretici."""
            ...
            try:
                ...
                if result and result.get("test_code"):
                    ...
                    if self._validate_test_syntax(code, language):
                        ...
                        logger.warning(
                            "%s: Syntax dogrulamasi basarisiz — %s",
                            self.name, target.get("file_path", "?"),
                        )
                        # Syntax hatali kodu da dondur ama isaretli
                        result["estimated_coverage_gain"] = 0.0
                        result["syntax_valid"] = False
                        generated_tests.append(result)
            except Exception as exc:
                logger.warning(
                    "%s: Hedef icin test uretilemedi (%s): %s",
    ...
    def _generate_single_test(
        ...
        """Tek bir gap hedefi icin LLM'den test kodu uret."""
        ...
        # User prompt olustur
        user_prompt = (
            f"Asagidaki kapsanmayan kod icin {framework} test(ler)i yaz.\n\n"
            f"## Hedef Dosya\n{file_path}\n\n"
    ...
    def _validate_test_syntax(self, code: str, language: str) -> bool:
        ...
        # TypeScript/JavaScript icin basit sezgisel kontrol
    ...
    def _build_banking_context(self) -> str:
        ...
        return (
            ...
            "Kisisel veri maskeleme testleri ekle. TC Kimlik, IBAN, telefon vb.\n"
            ...
            "- Negatif senaryolar: Yetersiz bakiye, gecersiz IBAN, "
