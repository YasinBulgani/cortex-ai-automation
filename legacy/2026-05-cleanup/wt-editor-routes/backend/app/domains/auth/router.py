...
def logout_all(
    ...
    """Tum cihazlardan cikis yap — tum refresh token'lari iptal et."""
...
def forgot_password(request: Request, body: ForgotPasswordRequest, db: Annotated[Session, Depends(get_db)]):
    """Sifre sifirlama — token olustur ve teslim katmanina devret."""
    user = db.scalar(select(User).where(User.email == body.email))
    if user:
        reset_token = create_password_reset_token(user.id)
        delivered = _send_password_reset_email(user.email, reset_token)
        log_audit(
            db,
            actor_user_id=user.id,
            ...
            resource_type="user",
            resource_id=user.id,
            payload={"delivery": "smtp" if delivered else "disabled"},
            ip=_request_ip(request),
        )
        db.commit()
    # Guvenlik: Kullanici var ya da yok, ayni yanit
    return {"ok": True, "message": "Hesap varsa sifre sifirlama talimati gonderildi"}


@router.post("/reset-password")
@_limit(settings.rate_limit_register)
def reset_password(request: Request, body: ResetPasswordRequest, db: Annotated[Session, Depends(get_db)]):
    """Token ile sifre sifirlama."""
    user_id = verify_password_reset_token(body.token)
    if not user_id:
        raise HTTPException(400, "Gecersiz veya suresi dolmus token")
...
def register(request: Request, body: RegisterRequest, db: Annotated[Session, Depends(get_db)]):
    ...
    if not settings.allow_self_registration:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Acik kayit devre disi. Yoneticiyle iletisime gecin.",
        )
