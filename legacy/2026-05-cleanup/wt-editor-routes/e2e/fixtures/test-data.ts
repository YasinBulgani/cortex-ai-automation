...
export class TestDataFactory {
  ...
  async createUser(prefix = "e2e"): Promise<TestUser> {
    ...
    await this.request.post(`${API}/api/v1/auth/users`, {
      headers: {
        ...this.headers,
    ...
    const loginRes = await this.request.post(`${API}/api/v1/auth/login`, {
      data: { email, password },
    });
  ...
  async createProject(name?: string): Promise<TestProject> {
    ...
    const res = await this.request.post(`${API}/api/v1/tspm/projects`, {
  ...
  async createScenario(projectId: string, title?: string): Promise<TestScenario> {
    ...
    const res = await this.request.post(`${API}/api/v1/tspm/projects/${projectId}/scenarios`, {
      headers: this.headers,
  ...
  async createExecution(projectId: string, name?: string): Promise<string> {
    ...
    const res = await this.request.post(`${API}/api/v1/tspm/projects/${projectId}/executions`, {
      headers: this.headers,
      data: { name: execName },
    });
    const body = await res.json();
  ...
  async createApproval(projectId: string, title: string, scenarioId?: string): Promise<string> {
    const res = await this.request.post(`${API}/api/v1/tspm/projects/${projectId}/approvals`, {
      headers: this.headers,
    ...
    for (const item of this.cleanupIds.reverse()) {
      try {
        if (item.type === "project") {
          await this.request.delete(`${API}/api/v1/tspm/projects/${item.id}`, {
            headers: this.headers,
          });
        }
      } catch {
