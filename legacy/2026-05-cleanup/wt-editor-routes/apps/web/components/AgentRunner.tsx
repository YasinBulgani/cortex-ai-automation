...
export function AgentRunner() {
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "fixed right-0 top-0 z-50 flex h-full w-[480px] max-w-[90vw] flex-col border-l border-slate-800 bg-slate-900 shadow-2xl transition-transform duration-300",
      ...
      >
        ...
        <div className="border-b border-slate-800 px-4 py-3">
          <div className="flex items-center justify-between text-xs text-slate-400 mb-2">
            <span>İlerleme ({Math.min(Math.floor(progress / 10) + 1, 10)}/10 döngü)</span>
            ...
            {PHASE_ORDER.map((p, i) => {
              ...
              return (
                <div
                  key={p}
                  className={cn(
                    "flex-1 rounded px-2 py-1.5 text-center text-[10px] font-medium transition-colors",
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 font-mono text-xs">
          ...
          {logs.map((log, i) => (
            <div
              key={i}
              className={cn(
                "mb-1 flex gap-2 rounded px-2 py-1 transition-colors",
            ...
            >
              ...
              <span className="shrink-0 text-slate-400 w-[52px]">
              ...
              <span className="shrink-0 w-[120px] truncate text-slate-400" title={log.agent}>
                [{log.agent}]
              </span>
              <span className={cn("flex-1 break-all", LEVEL_COLORS[log.level] || "text-white")}>
                {log.message}
              </span>
            </div>
          ))}
