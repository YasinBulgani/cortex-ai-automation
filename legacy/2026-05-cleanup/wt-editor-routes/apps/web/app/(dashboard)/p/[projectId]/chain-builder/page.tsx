...
export default function ChainBuilderPage() {
  ...
  const handleRun = useCallback(() => {
    setRunOutput(null);
    const steps = nodes.map((n) => `[${n.data.method}] ${n.data.path}`);
    const output = [
      `Chain "${chainName}" calistiriliyor...`,
      `${nodes.length} adim, ${edges.length} baglanti`,
      "",
      ...steps.map((s, i) => `  ${i + 1}. ${s}`),
      "",
      "Chain calistirma istegi gonderildi.",
    ].join("\n");
    setRunOutput(output);
  }, [chainName, nodes, edges]);
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-4" data-testid="chain-builder-page">
      <PageHeader
        title="Chain Builder"
        description="Surukle-birak ile API cagri zincirleri olusturun ve test edin"
        badge={
          loadedChainId ? (
