        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4 overflow-auto">
          {!selected ? (
            ...
            <>
              ...
              {workspaceMode === "results" && runResults && (
                <SectionCard
                  title="Kosu Sonuclari"
                  ...
                  right={
                    <div className="flex gap-2 text-xs">
                      <span className="text-emerald-400">{runPassCount} gecti</span>
                      <span className="text-red-400">{runFailCount} basarisiz</span>
                    </div>
                  }
                  noPad
                >
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-slate-800">
                        <th className="px-4 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-slate-400">Istek</th>
                        <th className="px-4 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-slate-400">Durum Kodu</th>
                        <th className="px-4 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-slate-400">Sonuc</th>
                        <th className="px-4 py-2.5 text-right text-xs font-medium uppercase tracking-wide text-slate-400">Sure</th>
                      </tr>
                    </thead>
                    <tbody>
                      {requests.map((r) => (
                        <tr key={r.id} className="border-b border-slate-800 last:border-0 hover:bg-slate-800/30">
                          ...
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded border text-xs font-bold font-mono ${METHOD_COLORS[r.method] ?? ""}`}>{r.method}</span>
                          </td>
                          <td className="px-4 py-3 font-mono text-xs text-slate-400 max-w-xs truncate">{r.path}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
