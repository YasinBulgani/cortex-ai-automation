...
const STATUS_STYLES: Record<string, { color: string; dot: string; label: string }> = {
  healthy: { color: "bg-emerald-500/10 border-emerald-500/20 text-emerald-400", dot: "bg-emerald-400", label: "Sağlıklı" },
  broken:  { color: "bg-red-500/10 border-red-500/20 text-red-400",            dot: "bg-red-400",     label: "Kırık" },
  warning: { color: "bg-amber-500/10 border-amber-500/20 text-amber-400",      dot: "bg-amber-400",   label: "Uyarı" },
};
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6 flex flex-col gap-4" data-testid="locators-page">
      <PageHeader
        ...
        title="Locator Yönetimi"
        description="Element konumlandırıcılarını görüntüleyin"
      />

      {/* Stats */}
      <div className="grid grid-cols-3 gap-3" data-testid="locator-stats">
        <div className="rounded-xl border border-slate-700 bg-slate-900/40 px-4 py-3">
          ...
          <p className="text-2xl font-bold text-white">{locators.length}</p>
        </div>
        <div className="rounded-xl border border-slate-700 bg-slate-900/40 px-4 py-3">
          <p className="text-xs text-slate-400 mb-1">Sağlıklı</p>
          <p className="text-2xl font-bold text-emerald-400">{healthy}</p>
        </div>
        <div className="rounded-xl border border-slate-700 bg-slate-900/40 px-4 py-3">
          <p className="text-xs text-slate-400 mb-1">Kırık</p>
      ...
      <div className="rounded-xl border border-slate-700 bg-slate-900/40 overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="p-8">
            <EmptyState icon="🎯" title="Henüz locator yok" description="Element konumlandırıcıları ekleyin" />
          </div>
        ) : (
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-400">
          Mevcut locator&apos;larinizin stabilite analizini yapin. Kirik ve riskli selector&apos;lar otomatik tespit edilir.
        </p>
        <button
      ...
      {result && (
        <>
          ...
          <div className="grid grid-cols-5 gap-3">
            ...
            <StatCard label="Saglikli" value={result.healthy} color="text-emerald-400" />
            <StatCard label="Uyari" value={result.warning} color="text-amber-400" />
            <StatCard label="Kritik" value={result.critical} color="text-red-400" />
            <div className="rounded-xl border border-slate-700 bg-slate-900/40 px-4 py-3">
          ...
          >
            {result.details.length === 0 ? (
              <div className="p-8">
                <EmptyState icon="📊" title="Detay bulunamadi" description="Analiz sonucu bos dondu" />
              </div>
            ) : (
...
function TabFallback() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-slate-400">
        Kirik bir selector girip fallback zinciri ile cozum arayin. AI destekli stratejiler sirasi ile denenir.
      </p>

      {/* Input form */}
      <SectionCard
        title="Kirik Selector"
      ...
      >
        <form onSubmit={handleResolve} className="space-y-3">
          <div className="grid gap-3 sm:grid-cols-2">
            <input
              placeholder="Kirik Selector (orn. #old-login-btn)"
            ...
            />
            <input
              placeholder="Sayfa URL (istege bagli)"
              value={form.page_url}
          ...
          <textarea
            placeholder="DOM Snippet (istege bagli — ilgili HTML blogu)"
          ...
          <button type="submit" disabled={resolve.isPending || !form.broken_selector} className={BTN_PRIMARY}>
            ...
            {resolve.isPending ? "Cozumleniyor..." : "Cozumle"}
          </button>
        </form>
      </SectionCard>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className={`text-sm font-semibold ${result.success ? "text-emerald-300" : "text-red-300"}`}>
                    {result.success ? "Cozum bulundu!" : "Cozum bulunamadi"}
                  </p>
...
function TabPOM({ locators }: { locators: Locator[] }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-slate-400">
        Page Object Model (POM) sinifi uretin. Element bilgilerini JSON olarak girin, session ID ile otomatik cekin veya mevcut locator&apos;larinizi kullanin.
      </p>
...
function TabBreakage({ locators }: { locators: Locator[] }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-slate-400">
        Locator&apos;larinizin kirilma riskini tahmin edin. Git diff yapistirarak son degisikliklere gore analiz yapilabilir.
      </p>
      ...
      >
        <div className="space-y-3">
          <textarea
            placeholder="Son degisiklikler (git diff yapistirin — istege bagli)"
            value={recentChanges}
      ...
      {predictions && (
        <>
          <div className="grid grid-cols-3 gap-3">
            <StatCard label="Yuksek Risk" value={highRisk} color="text-red-400" />
            <StatCard label="Orta Risk" value={mediumRisk} color="text-amber-400" />
            <StatCard label="Dusuk Risk" value={lowRisk} color="text-emerald-400" />
          </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6 flex flex-col gap-4" data-testid="locators-page">
      <PageHeader
        ...
        description="Element konumlandiricilarini yonetin, stabilite analizi yapin ve AI destekli cozumler kullanin"
        badge={
