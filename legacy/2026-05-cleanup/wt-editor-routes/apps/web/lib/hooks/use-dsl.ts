...
export function useDslActions(filters: DslListFilters = {}) {
  const { category, lang, tag, page = 1, page_size = 50 } = filters;

  return useQuery({
    queryKey: dslKeys.actionList({ category, lang, tag, page, page_size }),
    queryFn: () => {
...
/**
 * Arama sonucu için kullanıcı geri bildirimi (👍 / 👎).
