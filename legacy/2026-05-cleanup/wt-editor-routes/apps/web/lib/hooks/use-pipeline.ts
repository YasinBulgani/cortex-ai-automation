...
export function useBankingHealth() {
  return useQuery({
    ...
    staleTime: 30 * 1000, // 30 sn — saglik durumu sik degisir
...
export function usePipelineStatus(enabled = true) {
  return useQuery({
    ...
    queryFn: () =>
      apiFetch<PipelineSnapshot>("/api/v1/agents/banking/pipeline/status"),
    enabled,
    refetchInterval: (query) => {
      // Pipeline calismiyorsa polling'i yavaslat
...
export function useStartPipeline() {
  ...
  return useMutation({
    mutationFn: (data: PipelineStartRequest) =>
      apiFetch("/api/v1/agents/banking/pipeline/start", {
        method: "POST",
        json: data,
      }),
    onSuccess: () => {
...
export function useCancelPipeline() {
  ...
  return useMutation({
    mutationFn: () =>
      apiFetch("/api/v1/agents/banking/pipeline/cancel", { method: "POST" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: pipelineKeys.status() });
    },
  });
