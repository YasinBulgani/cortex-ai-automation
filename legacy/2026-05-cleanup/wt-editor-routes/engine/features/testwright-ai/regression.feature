...
Feature: Regresyon Test Seti
  ...
  Scenario: REG-AUTH-003 Oturum kapatma akışı
    When kullanıcı "[data-testid='user-menu']" seçicisini tıklar
    When kullanıcı "Çıkış Yap" metnine tıklar
  ...
  Scenario: REG-PRJ-001 Proje listesi erişimi
    ...
    Then "[data-testid='project-list']" elementi görünür olmalıdır

  Scenario: REG-PRJ-002 Yeni proje oluşturma
    Given kullanıcı "/projects" sayfasındadır
    When kullanıcı "Yeni Proje" metnine tıklar
    When kullanıcı "[data-testid='project-name']" kutusuna "Regresyon Test Projesi" yazar
    When kullanıcı "Oluştur" metnine tıklar
    Then "[data-testid='project-card']" elementi görünür olmalıdır

  Scenario: REG-PRJ-003 Proje dashboard erişimi
    Given kullanıcı "/projects" sayfasındadır
    When kullanıcı "[data-testid='project-card']" seçicisini tıklar
  ...
  Scenario: REG-SCN-005 Senaryo düzenleme
    ...
    When kullanıcı "[data-testid='scenario-title']" kutusuna "Güncellenmiş Regresyon Senaryosu" yazar
    When kullanıcı "Kaydet" metnine tıklar
    Then URL "/scenarios" içermelidir
  ...
  Scenario: REG-EXC-001 Koşu listesi erişimi
    Given kullanıcı test projesinin "scenarios" sayfasındadır
    When kullanıcı "Koşular" metnine tıklar
    Then "[data-testid='execution-list']" elementi görünür olmalıdır

  Scenario: REG-EXC-002 Koşu detayı görüntüleme
    Given kullanıcı test projesinin "scenarios" sayfasındadır
    When kullanıcı "Koşular" metnine tıklar
    When kullanıcı "[data-testid='execution-row']" seçicisini tıklar
    Then "[data-testid='execution-detail']" elementi görünür olmalıdır
  ...
  Scenario: REG-REG-001 Regresyon seti listesi
    Given kullanıcı test projesinin "scenarios" sayfasındadır
    When kullanıcı "Regresyon Setleri" metnine tıklar
    Then "[data-testid='regression-set-list']" elementi görünür olmalıdır
  ...
  Scenario: REG-DATA-001 Veri seti sayfası erişimi
    ...
    Then "[data-testid='dataset-list'], [data-testid='upload-area']" elementi görünür olmalıdır
