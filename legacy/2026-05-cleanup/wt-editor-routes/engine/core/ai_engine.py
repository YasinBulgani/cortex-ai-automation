...
Desteklenen sağlayıcılar: OpenAI, DeepSeek, Together.ai, Ollama (OpenAI uyumlu herhangi bir API) ve Anthropic (Claude ailesi)
"""
import json
import re
...
class AIEngine:
    ...
    def client(self):
        if not self._client:
            if settings.OPENAI_MODEL.startswith("g4f-"):
                from g4f.client import Client as G4FClient
                self._client = G4FClient()
            else:
                if not settings.OPENAI_API_KEY:
                    raise ValueError(
                        "OPENAI_API_KEY eksik! Lütfen .env dosyasına API anahtarınızı ekleyin."
                    )
                self._client = OpenAI(
    ...
    def _call_llm(self, messages: list[dict], temperature: float = 0.7) -> str:
        """Evrensel LLM Çağırıcı: Model ismine göre OpenAI veya Anthropic alt yapısını seçer."""
        is_g4f = settings.OPENAI_MODEL.startswith("g4f-")
        model = self.execute_model_name
        
        # Anthropic (Claude) API Desteği
        if not is_g4f and model.lower().startswith("claude"):
            if not self._anthropic_client:
                import anthropic
                if not settings.ANTHROPIC_API_KEY:
                    raise ValueError("Claude modelini kullanmak için ANTHROPIC_API_KEY gereklidir!")
                self._anthropic_client = anthropic.Anthropic(api_key=settings.ANTHROPIC_API_KEY)
            
            # Anthropic mesaj yapısına uygun dönüştür: System mesajları özel parametre olarak verilir
            system_prompt = ""
            anthropic_msgs = []
            for msg in messages:
                if msg["role"] == "system":
        ...
        # Standart (OpenAI, DeepSeek, Together, Ollama, g4f) API Desteği
