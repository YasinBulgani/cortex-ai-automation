...
def _generate_analysis(actions_log, console_errors, network_errors,
    ...
    """Monkey test sonuçlarından analist raporu üretir."""
    scenarios = []
    bugs = []
    recommendations = []
    ...
    click_errors = [a for a in actions_log if a.get("result", "").startswith("click_error")]
    fill_errors = [a for a in actions_log if a.get("result", "").startswith("fill_error")]
    nav_errors = [a for a in actions_log if a.get("result", "").startswith("nav_error")]

    if click_errors:
        scenarios.append({
            "title": "Tıklanamayan Element Testi",
            "type": "functional",
            "description": f"{len(click_errors)} element tıklanamadı. Disabled, overlapping veya JavaScript hatası olabilir.",
            "steps": [
    ...
    if fill_errors:
        scenarios.append({
            "title": "Form Alanı Güvenlik Testi",
            "type": "security",
            "description": f"{len(fill_errors)} input alanında sorun tespit edildi.",
            "steps": [
                {"action": "Tüm input alanlarına uzun metin gir (500+ karakter)", "expected": "Uygulama çökmemeli, karakter limiti olmalı"},
    ...
    if nav_errors:
        scenarios.append({
            "title": "Navigasyon ve Routing Testi",
            "type": "functional",
    ...
    if any(e.get("status") == 401 for e in network_errors):
        scenarios.append({
            "title": "Oturum ve Yetkilendirme Testi",
            "type": "security",
    ...
    if console_errors:
        ...
        if js_errors:
            scenarios.append({
                "title": "JavaScript Hata Regresyon Testi",
                "type": "regression",
                "description": f"{len(js_errors)} JavaScript hatası tespit edildi. Her biri fix sonrası regresyon testi gerektirir.",
                "steps": [
    ...
    scenarios.append({
        "title": "Stres Altında Stabilite Testi",
        "type": "performance",
    ...
    if critical_count > 0:
        recommendations.append({
            "priority": "critical",
            "text": f"{critical_count} kritik hata öncelikle düzeltilmeli. Bunlar güvenlik açığı veya uygulama çökmesine neden olabilir.",
        })
    if warning_count > 3:
        recommendations.append({
            "priority": "high",
            "text": f"{warning_count} uyarı seviyesinde sorun var. Console hatalarını temizlemek kullanıcı deneyimini iyileştirir.",
        })
    if stability_score < 50:
        recommendations.append({
            "priority": "critical",
            "text": f"Stabilite skoru %{stability_score} — uygulama kararsız. Kapsamlı hata ayıklama gerekli.",
        })
    elif stability_score < 80:
        recommendations.append({
            "priority": "high",
            "text": f"Stabilite skoru %{stability_score} — iyileştirme gerekli. Tespit edilen hatalar düzeltilmeli.",
        })
    else:
        recommendations.append({
            "priority": "info",
            "text": f"Stabilite skoru %{stability_score} — uygulama genel olarak kararlı.",
        })

    if any("Güvenlik" in b["category"] or "security" == s.get("type") for b in bugs for s in scenarios):
        recommendations.append({
            "priority": "critical",
            "text": "Güvenlik ile ilgili bulgular var. Penetrasyon testi ve güvenlik denetimi önerilir.",
        })
    ...
    if perf_metrics:
        ...
        if avg_load > 3000:
            recommendations.append({
                "priority": "high",
                "text": f"Ortalama sayfa yükleme süresi {avg_load:.0f}ms — performans optimizasyonu gerekli.",
            })
...
def api_monkey_run():
    """
    Gelişmiş Monkey Testing — rastgele etkileşim, performans ölçümü,
    hata kategorilendirme ve otomatik senaryo üretimi.
    """
    ...
    weights = {k: v for k, v in ACTION_WEIGHTS.items() if k in enabled_actions}
    if not weights:
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            if credentials and credentials.get("login_url"):
                try:
                    ...
                    if credentials.get("username_selector") and credentials.get("username"):
                        page.fill(credentials["username_selector"],
                                  credentials["username"])
                    if credentials.get("password_selector") and credentials.get("password"):
                        page.fill(credentials["password_selector"],
            ...
            perf_metrics.append({
                "url": url,
                "load_time_ms": round(initial_load_ms),
                "timestamp": datetime.now().isoformat(),
            })

            error_before = len(console_errors) + len(network_errors)
            ...
            for i in range(max_actions):
                ...
                try:
                    if action_type == "click":
                        _do_click(page, record)
                    elif action_type == "fill":
                        _do_fill(page, record)
                    elif action_type == "scroll":
                        _do_scroll(page, record)
                    elif action_type == "navigate":
                        _do_navigate(page, record, pages_visited, perf_metrics, url)
                    elif action_type == "hover":
                        _do_hover(page, record)
                    elif action_type == "double_click":
                        _do_double_click(page, record)
                    elif action_type == "right_click":
                        _do_right_click(page, record)
                    elif action_type == "keyboard":
                        _do_keyboard(page, record)
                    elif action_type == "tab_navigation":
                        _do_tab_navigation(page, record)
                    elif action_type == "select_change":
                        _do_select_change(page, record)
                    elif action_type == "checkbox_toggle":
                        _do_checkbox_toggle(page, record)
                    elif action_type == "resize_viewport":
                        _do_resize(page, context, record)
                    elif action_type == "back_forward":
                        _do_back_forward(page, record)

                    error_after = len(console_errors) + len(network_errors)
                    if error_after > error_before:
                        record["triggered_error"] = True
                        try:
                            ss = page.screenshot(full_page=False)
                            screenshots[f"error_step_{i+1}"] = base64.b64encode(ss).decode()
                        except Exception:
                            pass
        ...
        stability_score = max(0, 100 - (error_count * 3) -
                              (len([a for a in actions_log
                                    if "error" in (a.get("result") or "")]) * 2))
...
def _do_click(page, record):
    ...
    visible = [e for e in clickables if e.is_visible()]
    if visible:
...
def _do_select_change(page, record):
    ...
    visible = [s for s in selects if s.is_visible()]
    if visible:
        chosen = random.choice(visible)
        options = chosen.evaluate("""
...
def _do_checkbox_toggle(page, record):
    checks = page.query_selector_all(
        "input[type='checkbox'], input[type='radio']"
    )
    visible = [c for c in checks if c.is_visible()]
    if visible:
        chosen = random.choice(visible)
        chosen.click(timeout=3000)
...
def _do_resize(page, context, record):
    ...
    context.pages[0].set_viewport_size(size)
    record["viewport"] = f"{size['width']}x{size['height']}"
