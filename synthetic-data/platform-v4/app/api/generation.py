...
"""
    ...
    try:
        ...
        if api_key:
            ...
            # 2. Deneme: AI Gateway (vLLM → Groq → Gemini → Ollama fallback zinciri)
            try:
                import httpx as _httpx
                _gw_base = os.getenv("AI_GATEWAY_BASE_URL", "http://ai-gateway:8080").rstrip("/")
                _gw_key = os.getenv("GATEWAY_INTERNAL_KEY", "")
                _headers = {"X-Internal-Key": _gw_key} if _gw_key else {}
                _resp = _httpx.post(
                    f"{_gw_base}/ai/complete",
                    json={
                        "task_type": "chat",
                ...
                llm_response = _resp.json().get("content", "")
            except Exception as gw_err:
                logging.warning(f"AI Gateway hatası: {gw_err}. Fallback regex kullanılıyor.")
