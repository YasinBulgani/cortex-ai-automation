...
# ── PII desenleri ────────────────────────────────────────────────────────
# TEK KAYNAK: backend/app/domains/ai/pii_redactor.py
# Engine bağımsız deploy edilebildiği için pattern'leri local tutuyoruz ama
# placeholder formatı ve semantik backend ile İDENTİK (geriye dönük uyum).
# Yeni pattern eklemek istiyorsan: pii_redactor.py güncelle + aşağıdaki
# listeyi senkronla. CI arch-guard drift'i yakalayacak (Dalga 0, E3).
_DEFAULT_PII_PATTERNS: list[tuple[str, str]] = [
    (r"\b[Tt][Rr]\d{2}\s?\d{4}\s?\d{4}\s?\d{4}\s?\d{4}\s?\d{4}\s?\d{2}\b", "[IBAN]"),
    (r"\b(?:\d[ -]?){13,19}\b", "[KART]"),
    (r"(?:\+90|0)?\s?5\d{2}[\s-]?\d{3}[\s-]?\d{2}[\s-]?\d{2}\b", "[TELEFON]"),
...
# TEK KAYNAK: ``infra/registry/model_registry.yaml``.
# Eski hardcoded dict kaldırıldı (2026-Q2, Dalga 0 konsolidasyonu).
# Runtime'da tabloya yazan kod (ör. gateway'den model listesi push) hâlâ
# çalışır: build_legacy_model_costs() base dict döner, üstüne mutate edilir.
from ._model_registry import build_legacy_model_costs as _build_legacy

MODEL_COSTS: dict[str, dict[str, float]] = _build_legacy()
...
# ── Model adı → AI Gateway task_type eşlemesi (proxy mode için) ──
# ADR-0002 Faz 4.C: engine doğrudan OpenAI/Anthropic yerine ai-gateway'e
# gidebilsin diye. Model adı task_type tahmini sadece fallback; ideal olarak
# caller `gateway_task_type` kwarg'ı ile explicit ver.
_MODEL_TO_TASK_TYPE: dict[str, str] = {
    "gpt-4o": "chat",
    "gpt-4o-mini": "chat",
    "claude-3-5-sonnet-latest": "chat",
    "claude-3-5-haiku-latest": "chat",
    "qwen2.5-coder:7b": "generate_playwright",
    "qwen2.5:14b": "analyze_document",
}
...
def gateway_proxy_default() -> bool:
    return _env_truthy("ENGINE_LLM_USE_GATEWAY", default=_is_production_env())


def direct_providers_allowed() -> bool:
    return _env_truthy(
        "ENGINE_LLM_ALLOW_DIRECT_PROVIDERS",
...
class LLMGateway:
    ...
    İki mod:
      1. **Direct mode** (sadece dev/test): OpenAI/Anthropic SDK'sı ile doğrudan çağırır.
      2. **Proxy mode** (env `ENGINE_LLM_USE_GATEWAY=1`): AI Gateway servisine
         HTTP çağrısı yapar. Tek merkezli budget/rate/fallback için önerilir.

    Proxy mode'u aktif etmek için ortam değişkenleri:
        ENGINE_LLM_USE_GATEWAY=1
        AI_GATEWAY_BASE_URL=http://ai-gateway:8080   (varsayılan)
    ...
    def __init__(
        ...
        use_gateway_proxy: bool | None = None,
    ):
        ...
        # Proxy mode: explicit arg > env var > production default.
        if use_gateway_proxy is None:
            self.use_gateway_proxy = gateway_proxy_default()
        else:
            self.use_gateway_proxy = use_gateway_proxy
        self.direct_providers_allowed = direct_providers_allowed()
        self._gateway_base_url = os.getenv(
            "AI_GATEWAY_BASE_URL", "http://ai-gateway:8080"
        ).rstrip("/")
        self._gateway_key = os.getenv("GATEWAY_INTERNAL_KEY", "")
        if self.use_gateway_proxy:
            logger.info(
                "LLMGateway proxy mode aktif → %s (PII sanitize hala engine-side)",
                self._gateway_base_url,
            )
        elif not self.direct_providers_allowed:
            raise RuntimeError(
                "Engine direct LLM provider mode is disabled. "
                "Set ENGINE_LLM_USE_GATEWAY=1 or explicitly allow dev-only direct mode "
                "with ENGINE_LLM_ALLOW_DIRECT_PROVIDERS=1."
            )
    ...
    def complete(
        ...
        if self.use_gateway_proxy:
            response = self._call_ai_gateway(messages, model, temperature, max_tokens)
    ...
    def _call_ai_gateway(
        ...
        """AI Gateway'e HTTP proxy — fallback zinciri: Groq → Gemini → Ollama → g4f.

        Faz 4.C (ADR-0002): Engine'in doğrudan OpenAI/Anthropic çağrısı yapması
        yerine ai-gateway mikroservisini kullanması. Bu sayede:
          - Tek merkezli provider fallback (ai-gateway tarafından yönetilir)
          - PII sanitize engine-side zaten yapıldı (double-safety)
          - Budget/rate engine-side tutulmaya devam ediyor (self.stats)
        """
        try:
            import httpx  # lazy import — proxy mode kapalıysa bağımlılık yüklenmez
        except ImportError as exc:
            raise RuntimeError(
                "ENGINE_LLM_USE_GATEWAY=1 için httpx kütüphanesi gerekli"
            ) from exc

        task_type = _MODEL_TO_TASK_TYPE.get(model, "chat")
        payload = {
            "task_type": task_type,
        ...
        url = f"{self._gateway_base_url}/ai/complete"

        try:
            resp = httpx.post(url, json=payload, headers=headers, timeout=90.0)
            resp.raise_for_status()
        except httpx.HTTPError as exc:
            logger.error("AI Gateway HTTP hatası: %s", exc)
            raise RuntimeError(f"AI Gateway erişilemiyor: {exc}") from exc
        ...
        # AI Gateway kendi cost hesabını yapmıyor — engine-side tahmin
        cost = 0.0
        try:
            # input/output split yok; yarıya böl konservatif tahmin
            half = tokens // 2
            cost = self._calculate_cost(model_used, half, tokens - half)
        except Exception:
            pass
