...
def _get_llm_client():
    """
    Sırayla dene:
    1. Merkezi LLMGateway
    2. Ollama yerel HTTP (localhost:11434)
    """
    try:
        ...
        if gateway.available:
            return gateway, os.getenv('OPENAI_MODEL', 'gpt-4o-mini'), 'gateway'
    except Exception as exc:
        logger.debug("LLMGateway kullanılamıyor, yerel Ollama deneniyor: %s", exc)

    # 2. Ollama (yerel — API key gerektirmez)
