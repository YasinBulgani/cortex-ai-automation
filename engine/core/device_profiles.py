...
class DeviceProfile:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.platform == "ios":
            os_str = f"iOS {self.farm_os_version}" if self.farm_os_version else "iOS"
            icon = "🍎"
        else:
            os_str = f"Android {self.farm_os_version}" if self.farm_os_version else "Android"
            icon = "🤖"
        ...
        if self.playwright_key:
            result["playwright_key"] = self.playwright_key
        return result
