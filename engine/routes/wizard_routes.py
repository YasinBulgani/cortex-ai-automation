...
def api_discover_selectors():
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # Gereksiz ağır kaynakları engelle — crawl hız artışı ~3-5x
            context.route("**/*.{png,jpg,jpeg,gif,webp,svg,ico,woff,woff2,ttf,otf,mp4,webm}", lambda route: route.abort())
            context.route("**/*.{css}", lambda route: route.abort())
            page = context.new_page()
            page.goto(url, wait_until="domcontentloaded", timeout=15000)
            # Kısa bir DOM yerleşme süresi — SPA'lar için
