...
def register_editor_routes(app):
    ...
    def editor_ai_assist():
        """Kod parçasını LLM ile analiz et — prod'da AI Gateway zorunlu."""
        data = request.get_json(silent=True) or {}
        ...
        def stream_via_ai_gateway() -> Iterator[str]:
            try:
                import httpx
                from services.llm_gateway import gateway_proxy_default
        ...
        def generate():
            try:
                ...
                from services.llm_gateway import direct_providers_allowed, gateway_proxy_default

                if gateway_proxy_default():
                    yield from stream_via_ai_gateway()
                    return
                if not direct_providers_allowed():
                    yield f"data: {json.dumps({'type':'error','msg':'Direct LLM provider mode is disabled; enable AI Gateway proxy.'})}\n\n"
                    return
