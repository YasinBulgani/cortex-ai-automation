...
def manual_to_automation():
    ...
    # `allow_mock` (default True): AI anahtarı yoksa mock çıktı üret.
    # CI / üretim istemcileri `allow_mock=false` geçerek placeholder kod
    # üretimi yerine 503 almayı tercih edebilir (sessiz geçişin önüne geç).
    allow_mock: bool = bool(data.get("allow_mock", True))
    use_mock = not _has_ai_key()

    if use_mock and not allow_mock:
        return (
            jsonify(
                {
                    "ok": False,
    ...
    result: dict = {
        "ok": True,
        "mock_mode": use_mock,
        # `simulated` tüm pipeline yollarında tutarlı bir ad — fallback/mock
        # çıktılarını bu bayrakla ayırt ediyoruz (bkz. backend tarafı).
        "simulated": use_mock,
...
def preview_gherkin():
    ...
    use_mock = not _has_ai_key()
    allow_mock = bool(data.get("allow_mock", True))
            ...
            jsonify(
                {
                    "ok": False,
                    "error": (
                        "AI anahtarı yok ve `allow_mock=false`. Preview üretilemiyor."
                    ),
                    "mock_mode": True,
                    "simulated": True,
                    "provenance": _resolve_provenance(simulated=True),
