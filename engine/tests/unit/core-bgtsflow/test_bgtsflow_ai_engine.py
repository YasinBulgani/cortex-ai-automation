...
def test_execute_model_name(engine, monkeypatch):
    """Model ismi ayardan doğrudan okunur."""
    import core.ai_engine as _ai_mod

    monkeypatch.setattr(_ai_mod.settings, "OPENAI_MODEL", "gpt-4")
    assert engine.execute_model_name == "gpt-4"

    monkeypatch.setattr(_ai_mod.settings, "OPENAI_MODEL", "qwen2.5:14b")
    assert engine.execute_model_name == "qwen2.5:14b"
