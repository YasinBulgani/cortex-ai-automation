# ============================================================
#  Cortex Otomasyon — Windows Dependency Installer
# ============================================================
...
Header "Cortex Otomasyon — Windows Setup"
...
$mvnw = Join-Path $InstallDir "mvnw.cmd"
...
& $pip install --quiet -r (Join-Path $InstallDir 'python_server\requirements.txt')

# Train ML model if missing
$modelPath = Join-Path $InstallDir 'python_server\final_model.pkl'
if ((Test-Path (Join-Path $InstallDir 'python_server\train_model.py')) -and (-not (Test-Path $modelPath))) {
    Log-Info "ML modeli eğitiliyor..."
    Push-Location (Join-Path $InstallDir 'python_server')
    & $py train_model.py
    Pop-Location
}
Pop-Location
...
$launcherContent = @"
@echo off
title Cortex Otomasyon Dashboard
cd /d "$InstallDir"
...
# Desktop shortcut
$shortcut = Join-Path ([Environment]::GetFolderPath('Desktop')) 'Cortex Otomasyon.lnk'
$ws = New-Object -ComObject WScript.Shell
$lnk = $ws.CreateShortcut($shortcut)
$lnk.TargetPath = $launcher
$lnk.WorkingDirectory = $InstallDir
$lnk.Description = 'Cortex Otomasyon Dashboard'
...
Log-Info "Başlatmak için: çift tıklayın → Masaüstü\Cortex Otomasyon"
