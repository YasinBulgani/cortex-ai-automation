...
def cortex_recorder_enhance_stream():
    """Streaming version of /enhance (E14 fix — gerçek progress).

    Ollama'nın native stream API'sini (NDJSON satır satır token) SSE'ye
    forward eder. Dashboard EventSource ile dinleyip her token'ı text-area'ya
    yazabilir → polishElapsed counter yerine GERÇEK progress.

    Body: { featurePath?, featureContent?, model? }
    Stream events:
      data: {"type":"token", "text":"...", "elapsed_ms":1234}
      data: {"type":"done",  "final":"<full text>", "elapsed_ms":34567}
    ...
    if not feature_content and feature_path:
        try:
            ...
            def _err_gen():
                yield "data: " + json.dumps({"type": "error", "error": f"read fail: {e}"}) + "\n\n"
            return Response(stream_with_context(_err_gen()), mimetype="text/event-stream")

    if not feature_content.strip():
        def _err_gen():
            yield "data: " + json.dumps({"type": "error", "error": "feature content boş"}) + "\n\n"
        return Response(stream_with_context(_err_gen()), mimetype="text/event-stream")
    ...
    def _token_stream():
        ...
        try:
            with _urlreq.urlopen(req, timeout=180) as resp:
                # Ollama streams NDJSON, one chunk per line
    ...
    return Response(stream_with_context(_token_stream()),
