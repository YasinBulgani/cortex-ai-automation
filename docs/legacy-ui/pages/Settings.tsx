...
const Settings: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    if (settings.api.timeout < 1000 || settings.api.timeout > 300000) {
      setValidationError('API Timeout 1000–300000 ms arasında olmalıdır.');
      setActiveTab('api');
      return;
    }
    if (settings.api.retryAttempts < 0 || settings.api.retryAttempts > 10) {
      setValidationError('Retry Attempts 0–10 arasında olmalıdır.');
      setActiveTab('api');
      return;
    }
    if (settings.testing.defaultTimeout < 1000 || settings.testing.defaultTimeout > 600000) {
      setValidationError('Test Timeout 1000–600000 ms arasında olmalıdır.');
      setActiveTab('testing');
      return;
    }
    if (settings.testing.parallelWorkers < 1 || settings.testing.parallelWorkers > 16) {
      setValidationError('Paralel Worker sayısı 1–16 arasında olmalıdır.');
      setActiveTab('testing');
      return;
    }
  ...
  const parsePositiveInt = (value: string, min: number): number => {
            ...
            <div className="setting-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings((prev) => ({
                    ...prev,
                    api: { ...prev.api, timeout: parsePositiveInt(e.target.value, 1000) },
                  }))
                }
              />
              <p className="setting-help">Maximum time to wait for API responses (1000–300000 ms)</p>
            </div>
            ...
            <div className="setting-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings((prev) => ({
                    ...prev,
                    api: { ...prev.api, retryAttempts: parsePositiveInt(e.target.value, 0) },
                  }))
                }
              />
              <p className="setting-help">Number of times to retry failed requests (0–10)</p>
            </div>
          </div>
        )}
          ...
          <div className="settings-section">
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings((prev) => ({
                    ...
                    testing: {
                      ...prev.testing,
                      defaultTimeout: parsePositiveInt(e.target.value, 1000),
                    },
                  }))
                }
              />
              <p className="setting-help">Default timeout for test steps (1000–600000 ms)</p>
            </div>
            ...
            <div className="setting-group">
              ...
              <input
                ...
                min={1}
                max={16}
                value={settings.testing.parallelWorkers}
                onChange={(e) =>
                  setSettings((prev) => ({
                    ...prev,
                    testing: {
                      ...prev.testing,
                      parallelWorkers: parsePositiveInt(e.target.value, 1),
                    },
                  }))
                }
              />
              <p className="setting-help">Number of parallel test executions (1–16)</p>
            </div>
